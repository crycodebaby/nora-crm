...
| **CONDITIONAL CURRENT CONTRACT** | die Subsystemdokumente aus der Tabelle „Architekturbereiche" — darunter [`22`](22-security-and-access.md) (Security / Access / RBAC / RLS / Grants / Authorization), [`23`](23-operations-errors-feedback.md) (Operationen / Fehler / Feedback, **sektionsweise** §0–§6) und [`24`](24-pwa-and-update-lifecycle.md) (PWA / Service Worker / Update-Lifecycle, **sektionsweise** §0–§9) und [`25`](25-universal-work-model.md) (Universal Work Model v1 — Work, Arbeitskorb, `tasks` als Work Carrier) | wenn das Subsystem betroffen ist |
...
Bei allen Änderungen an **Aufgaben (`public.tasks`), dem Arbeitskorb, Work-Read-Models, Zuständigkeit/Holder, Wiedervorlage, Work Commands oder MCP-/Automation-Work-Tools** ist [`25-universal-work-model.md`](25-universal-work-model.md) **Pflicht** — der eingefrorene Domain-Contract für Work. Er entscheidet, was überhaupt ein Arbeitsgegenstand ist, welche Tabelle ihn tragen darf, welche Zustände und Fälligkeitssemantik gelten, wer als Actor gilt und was der Arbeitskorb standardmäßig liefert. Suchbegriffe, die dorthin führen: Work · Work Carrier · Carrier Capability · Row Validity · Arbeitskorb · `tasks` · Holder · Zuständigkeit · unassigned · Claiming · Assignment · Wiedervorlage · `deferred` · `due_precision` · `overdue` · `due_today` · Attention · `actionable` · `allowed_actions` · Application Query · `work.*` · W-A. **Wer ein neues Feld, einen neuen Zustand oder eine neue Aufgabenquelle einführen will, liest ihn zuerst** — mehrere naheliegende Erweiterungen (`deal_id` an Aufgaben, `cancel`, Checklistenpunkte als Arbeit, `allowed_actions`) sind dort ausdrücklich abgelehnt oder an ein benanntes Decision Gate gebunden.
...
| [`22-security-and-access.md`](22-security-and-access.md) | **globaler Security- und Access-Contract**: Enforcement-Prinzip, Authentication vs. Authorization, Rollen (`sales.role`) und Capability-Rollen, globale Berechtigungsmatrix, Trust Boundaries, Database Enforcement (RLS, Grants, Default-Privilegien, neue `public`-Objekte, `SECURITY DEFINER`, privilegierte Read-Views, `nora_private`, PostgREST-Exposure), Session- und Executor-Integrität — **inkl. der Anhang-Flächen**: Bucket `attachments` (§6.5), Metadatentabelle `public.attachments` (§6.6), Löschintent-Warteschlange (§6.7), Anhang-Liveness-Resolver (§6.8), Ausführungsvertrag der Warteschlange (§6.9), Serialisierung je Objektschlüssel samt Referenz-Zulassung (§6.10), Projektion der Notiz-Anhänge samt Anhang-Grammatik (§6.11) und Lese-Gate samt Schreibwächter und Darstellungsgrenze (§6.12) — **sowie der Work-Lesefläche**: Application Query, Actor-Resolver und die `INVOKER`/`DEFINER`-Aufteilung (§6.13) |
...
| [`25-universal-work-model.md`](25-universal-work-model.md) | **eingefrorener Domain-Contract für Work** (Status `FROZEN`, 2026-09-21): Work vs. Work Carrier, Carrier Capability vs. Row Validity, `public.tasks` als einziger Work Carrier v1 (und warum Checklisten keiner sind), Lifecycle (`open`/`done`; `deferred` als spätere Zielsemantik, `cancel` nicht in v1), Fälligkeit (`due_at` nullable, `due_precision`, `Europe/Berlin`), Holder-/Zuständigkeitsmodell, Actor aus der Session und Query Subject ≠ Security Actor, Claim/Assign-Zielrichtung, Core vs. Contributed Actions, `allowed_actions`, Kontextmodell, Team-Definition, Attention / Derived Read State, **Arbeitskorb-Default-Query**, LLM/MCP-Readiness sowie Scope, PASS/FAIL und **Umsetzungsstand** der Welle **W-A** (`CLOSED` / `PRODUCTION VERIFIED` 2026-09-22: Query `public.get_work_items(...)`, Resolver `nora_private.current_sales_id()`, D-57 und D-53 geschlossen, **keine Work-Oberfläche**). **Keine** Security-Durchsetzung (→ [`22`](22-security-and-access.md) §6.13), **kein** Audit-Vertrag (→ [`13`](13-crm-audit-retention.md)), **keine** Darstellung (→ [`02`](02-design-system.md)) |
...
| **Work / Arbeitskorb / Aufgaben als Arbeitsgegenstand** (Work Carrier, Zuständigkeit/Holder, Wiedervorlage, Fälligkeitssemantik, Work Read Models, Work Commands, MCP-Work-Tools) | [`25`](25-universal-work-model.md) · Durchsetzung: [`22`](22-security-and-access.md) §6.13 | W-A Work Application Query · Universal Work Model v1 | G (G.7) | 5 · 15 |
...
| **Security / Access / Rollen / RBAC / RLS / Grants / Authorization** | [`22`](22-security-and-access.md) | W8-C S5 Verifiziertes Read Model der Notiz-Anhänge · W8-C S4 Backfill der Bestandsanhänge · W8-C S3B Projektion der Notiz-Anhänge · W8-C S3A Serialisierung je Objektschlüssel · W8-C S2A2.2 Ausführungsvertrag der Warteschlange · W8-C S2A2.1 Liveness-Resolver · W8-C S2A1 Lösch-Erfassung · W8-C S1 Attachment-Fundament · W8-B Attachment Hardening · Global Security & Access Owner · Wave 1 Default-Privilegien & Zielmatrix · Wave 0 TRUNCATE · RBAC/RLS v0.4b · v0.4b.1 · v0.4b.2 · Privilegierte Read-Views · Residual Advisor Closure | A · H (Anhänge/Storage) | 4 · 5 |
| Audit | [`13`](13-crm-audit-retention.md) | CRM-Audit v0.3l · Checklisten/Audit-Datenmodell 7b · W3 Audit-Actor · W8-C S2A2.1 (Audit hält keine Anhangdateien am Leben) | A · B | 7 |
...
| Was gilt als Arbeitsgegenstand, wer ist zuständig, was zeigt der Arbeitskorb? | Work Contract v1 — `public.tasks` ist der einzige Work Carrier; Arbeitskorb ist Read Model, nie eigene Persistenz | [`25`](25-universal-work-model.md) |
