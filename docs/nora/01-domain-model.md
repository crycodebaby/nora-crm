...
| Hauptansprechpartner | `contacts.is_primary` | Max. 1 pro `company_id` (Partial Unique Index). **Seit Atomic Contact Primary Intent (RC 2026-09-08) eine Geschäftstransition, kein Spaltenschreibvorgang:** jedes Kontakt-Speichern trägt eine explizite Absicht (`keep` · `make_primary` mit beobachtetem Halter · `clear`) und läuft als **eine** atomare Operation über `create_contact` / `update_contact`; `set_primary_contact` bleibt als schmaler Befehl auf demselben Transitionskern. Ein Kunde darf legitim **keinen** Hauptansprechpartner haben |
...
| Atomares Kontakt-Speichern | RPC `create_contact` / `update_contact` + Application Commands `executeCreateContact` / `executeUpdateContact` | Kontaktfelder, Kundenwechsel und Hauptansprechpartner-Transition in einer Transaktion; Kundenzeile gesperrt; beobachteter Halter wird verifiziert (`NORA_PRIMARY_CONTACT_CHANGED`); Legacy-Rohschreibpfad → `NORA_PRIMARY_CONTACT_ALREADY_EXISTS`. Beim Kundenwechsel reist die Rolle nie mit (`keep` → nicht primär beim Zielkunden); `company_id = null` ⇒ nie primär. Entscheidung: `06-decision-log.md` „2026-09-08 – Atomic Contact Primary Intent" (RC, nicht in Production) |
