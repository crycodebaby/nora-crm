...
- RLS ist aktiviert; genau **eine** permissive Policy `to authenticated`: `attachments_select_active_user` (`nora_private.is_active_user()`). **Keine** `INSERT`-, `UPDATE`-, `DELETE`- oder `ALL`-Policy.
- Grants: `revoke all` an `anon, authenticated, service_role`, danach genau `grant select … to authenticated`. `anon` und `service_role` halten **nichts**; `TRUNCATE`/`REFERENCES`/`TRIGGER`/`MAINTAIN` hält keine Rolle. `service_role` bleibt leer, weil es **keinen belegten, deployten Backend-Aufrufer** gibt — ein solcher Pfad braucht eigene Begründung, Zielmatrix-Eintrag und Assertion.
- **Kein direkter API-Schreibpfad — bewusst, nicht unfertig.** Ein direktes Zeilen-`DELETE` ließe die Tabelle von den Notiz-JSON-Arrays abweichen und erfasste ein Löschvorhaben für einen Schlüssel, der im JSON noch lebt; ein direktes `INSERT` schüfe Verweise, die das JSON nicht kennt. Der einzige Schreiber ist seit S3B die Datenbank selbst — die Projektion aus Abschnitt 6.11 —, nicht der Client. FK-`CASCADE`s sind davon unberührt: referenzielle Aktionen laufen als Tabellen-Owner.
- *Historisch:* S1 hatte `authenticated` zusätzlich `INSERT` und `DELETE` unter `nora_private.can_write()` gewährt (Policies `attachments_insert_writer` / `attachments_delete_writer`, begründet als Äquivalent zum Entfernen eines Verweises über das Notiz-Array). Beide Policies und beide Privilegien sind **seit S3A entfernt**; dieser Vertrag ist überholt ([`06`](06-decision-log.md) „2026-09-19 – W8-C S3A").
...
- **`CASCADE` löscht ausschließlich die Metadatenzeile, niemals das Storage-Objekt.** Keiner der Schnitte S1, S2A1, S2A2.1, S2A2.2, S3A und S3B enthält einen physischen Löschpfad; verwaiste Objekte bleiben liegen (akzeptierte Einschränkung seit W8-B, [`17`](17-known-issues-and-planned-waves.md) H.1). Nie als „die Datei wird gelöscht" formulieren. Seit S2A1 hängt an derselben Zeilenlöschung ein `AFTER DELETE`-Trigger, der ein Lösch**vorhaben** festhält — Abschnitt 6.7.
- `storage_key` ist `NOT NULL` und **`UNIQUE`** — eine provider-neutrale Objektidentität: kein `src`, keine öffentliche oder signierte URL, kein Hostname, kein Bucket- oder Anbietername wird hier persistiert. Die **Tabelle** trägt bewusst **keinen** Formatzwang, damit eine spätere Schlüsselstruktur (W8-E) möglich bleibt; welche Schlüsselform ein Notiz-Anhang heute haben darf, entscheidet die Grammatik der Projektion (Abschnitt 6.11), nicht ein Tabellen-Constraint.
- **`storage_key` ist nach dem Anlegen unveränderlich** (seit S3A, `BEFORE UPDATE`-Trigger, `55000` `NORA_ATTACHMENT_STORAGE_KEY_IMMUTABLE`) — auch für Owner- und manuelle Pfade, obwohl keine API-Rolle `UPDATE` hält. Ein Umschreiben A → B verschöbe einen Verweis ohne Erfassung und ohne Zulassung. Ein logischer Austausch ist **`DELETE` der alten plus `INSERT` einer neuen Zeile** und läuft damit durch Erfassung und Zulassungsprüfung (Abschnitt 6.10). Updates auf denselben Wert und auf andere Spalten (etwa der FK-`ON UPDATE CASCADE`) sind unberührt.
- **Jede neue Zeile durchläuft die Referenz-Zulassung** (seit S3A, `AFTER INSERT`-Trigger): ein Schlüssel mit aktivem Löschvorhaben oder `done` wird abgewiesen — Abschnitt 6.10.
- **Seit S6-A besitzt jede Zeile `ordinal integer NOT NULL` ohne Default.** `uq__attachments__owner_ordinal` ist eine nicht aufschiebbare Tabellen-Constraint `UNIQUE NULLS NOT DISTINCT (contact_note_id, deal_note_id, ordinal)`; wegen des XOR-Besitzes wäre eine gewöhnliche UNIQUE-Constraint mit `NULLS DISTINCT` wirkungslos. Die 31 Bestandszeilen wurden nach Legacy-Elementposition befüllt. Dieses Schema verbürgt eine Reihenfolge pro Notiz, aber die Anwendung liest und schreibt sie noch nicht als Autorität (S6-B1/B2 offen).
...
Stand seit W8-C S2A1 (`PRODUCTION VERIFIED` 2026-09-17, Migration `20260917120000_nora_attachment_deletion_capture`), Zustandsvokabular erweitert durch W8-C S2A2.1 (`PRODUCTION VERIFIED` 2026-09-18, Migration `20260918120000_nora_attachment_liveness_resolver`), datenbankinterner Ausführungsvertrag durch W8-C S2A2.2 (`PRODUCTION VERIFIED` 2026-09-18, Migration `20260918180000_nora_attachment_deletion_queue_execution`, Abschnitt 6.9), Erfassung je Schlüssel serialisiert durch W8-C S3A (`PRODUCTION VERIFIED` 2026-09-19, Abschnitt 6.10). **Dritte Fläche, dritter Contract:** 6.5 regelt den Bucket, 6.6 die Metadatentabelle, dieser Abschnitt die private Warteschlange, in der das *Vorhaben* einer Objektlöschung festgehalten wird.
...
- **Seit S3A nimmt die Erfassung vor dem Einfügen die Schlüsselsperre** (Abschnitt 6.10). Eine gleichzeitige Inspektion desselben Schlüssels ist damit entweder vorher fertig — dann sieht die Unterdrückung ihr Zurückziehen und ein frisches `pending` entsteht — oder sie wartet, bis die Löschung committet ist, und sieht den Schlüssel ohne diese Referenz. Das schließt den Verlust eines Vorhabens zwischen unterdrückter Erfassung und `live`-Inspektion (LOW-1). Die Erfassung läuft deshalb nur unter `READ COMMITTED`; unter einer höheren Isolationsstufe scheitert das Löschen einer Anhangzeile fail-closed.
...
- **Wer eine Erfassung auslöst:** jedes Zeilen-`DELETE` auf `public.attachments`. Seit S3A hält keine API-Rolle mehr ein direktes `DELETE` (Abschnitt 6.6). Heute entsteht ein Vorhaben über genau zwei reguläre Wege: die S3B-Projektion, wenn ein Anhang aus dem Array einer **projizierten** Notiz entfällt (Abschnitt 6.11), und den FK-`CASCADE` einer Notiz-, Kontakt-, Vorgangs- oder Kundenlöschung — dazu Owner-Pfade. Seit S3B ist die Erfassung damit **wirksam**, nicht mehr nur vorbereitet; für eine nie projizierte Bestandsnotiz existiert keine Zeile, also entsteht für sie kein Vorhaben ([`17`](17-known-issues-and-planned-waves.md) H.1). *Historisch* hielt Büro/Admin seit S1 ein direktes `DELETE`; S2A1 hat daran nichts geändert, S3A hat es entzogen. Die Policies aus 6.5 sind unberührt.

Regressionsprobe und operative Schritte: [`21`](21-agent-runbooks.md) Sektion 4.

### 6.8 Der Liveness-Resolver `nora_private.attachment_storage_key_liveness(text)`
...
> **`dead` ist eine Beobachtung, niemals eine Löscherlaubnis.** Das Ergebnis beschreibt, was **ein** Statement-Snapshot gesehen hat. Der Ausführungsvertrag (S2A2.2, Abschnitt 6.9) persistiert `dead` deshalb nicht, sondern gibt es nur an den Lease-Halter zurück. Seit S3A ruft die Inspektion den Resolver erst **unter der Schlüsselsperre** (Abschnitt 6.10): jede Löschung oder Zulassung einer `public.attachments`-Zeile für diesen Schlüssel ist dann entweder committet und sichtbar oder noch nicht begonnen. Das schließt das Re-Referenzierungs-Rennen für diese **zeilenbasierten** Verweise. Seit S3B gehören dazu auch Anhang-ändernde Notiz-Schreibvorgänge, weil sie über die Projektion Zeilen einfügen und löschen (Abschnitt 6.11). **Nicht** geschlossen ist es für Verweise, die an der Sperre nicht teilnehmen — Logos, Avatare, Konfiguration — und nicht für externe Seiteneffekte. Die Notiz-Arrays nehmen über die Projektion teil; seit dem S4-Backfill gilt das auch für den Bestand, der bei der Production-Verifikation am 2026-09-20 vollständig durch Zeilen vertreten war (datierte Beobachtung, kein Constraint). Ein künftiges Notiz-JSON ohne zugehörige Zeile entstünde nur außerhalb der Trigger. Physisch löschen darf weiterhin erst S2B ([`17`](17-known-issues-and-planned-waves.md) H.1).

**Tri-State-Semantik:**

| Ergebnis | Bedeutung |
|---|---|
| `live` | mindestens eine registrierte Fläche **beweist**, dass der Schlüssel referenziert wird. **`live` dominiert `unknown`** — ein Beweis ist ein Beweis, auch wenn eine andere Fläche mehrdeutig ist |
| `dead` | **alle** registrierten Flächen wurden erfolgreich ausgewertet, keine beweist `live`, und es trat **keine** Mehrdeutigkeit auf |
| `unknown` | kein `live`-Beweis, aber mindestens ein registrierter Wert hat eine Form, aus der sich `dead` nicht sicher ableiten lässt |

- **Ein Fehler ist kein Urteil.** Es gibt kein `exception when others`: Schema-Drift, ein fehlendes Objekt oder ein Codefehler bricht den Aufrufer ab und wird **nie** zu `unknown` oder `dead`. Ein ungültiger Schlüssel (`NULL`, leer, länger als 512 Zeichen) wirft `22023` `NORA_ATTACHMENT_LIVENESS_INVALID_KEY`; ein gültiger Schlüssel wird exakt verglichen, nie getrimmt.

**Registrierte Referenzflächen (Registry v1, bewusst fest codiert — keine Registry-Tabelle, kein dynamisches SQL):**

- `public.attachments.storage_key` (exakter Vergleich)
- `contact_notes.attachments` und `deal_notes.attachments` — beide **`jsonb[]`**, elementweise ausgewertet
- `companies.logo`
- `configuration`: die Branding-Logos `lightModeLogo` / `darkModeLogo` — **auch in der reinen URL-Form ohne `path`**, wie sie in Production vorkommt; sie nehmen an der Liveness vollwertig teil
- `configuration`-Rest (alles außer den beiden Branding-Schlüsseln) als **Sicherheits-Stolperdraht**: ein speicherartiger Wert oder der Kandidatenschlüssel selbst an unregistrierter Stelle ergibt `unknown` — nie `live`, nie eine stille Registrierung
- `contacts.avatar`
- `sales.avatar`

**Klassifikationsregeln mit dauerhafter Sicherheitswirkung:**

- **Fehlformen sind fail-closed.** Ein registrierter Wert mit unerwarteter Form (fehlerhafter `path`, Nicht-Objekt-JSON, eine `attachments`-artige Storage-URL außerhalb der kanonischen Form — signiert, authentifiziert, Render-Pfad, Query, Fragment, Prozent-Kodierung, zusätzliche Schrägstriche, relativ, gepolstert) ergibt `unknown`, nie `none`.
- **Unter den URL-Formen kann nur die kanonische öffentliche `attachments`-URL auf einem allowlisteten Origin `live` beweisen** (exakter Stringvergleich: das Production-Projekt und der lokale Supabase-Stack). Unabhängig davon beweisen ein exakter `public.attachments.storage_key` und ein verwendbarer `path`, der dem Schlüssel entspricht, `live`. Derselbe kanonische Schlüssel in URL-Form auf einem **anderen** Origin ergibt `unknown` — er kann weder `live` beweisen noch `dead` erlauben. URL-Referenzen, die auf einem neuen, noch nicht allowlisteten Origin (etwa einer eigenen Domain) gespeichert werden, sind deshalb fail-closed `unknown`, nicht fälschlich `dead`; `storage_key`-/`path`-Referenzen und URLs auf weiterhin erlaubten Origins bleiben davon unberührt. Ein neuer Origin wird nicht automatisch unterstützt — die Allowlist wird nur per Migration erweitert.
- **Fremde und nicht-Nora-Werte werden keine Referenz:** gewöhnliche fremde URLs, `data:`- und `blob:`-Werte, leere Werte und JSON-`null` zählen als „keine Referenz" — sie machen keinen Schlüssel `live` und keinen Schlüssel `unknown`.
- **Audit-Historie nimmt nicht teil.** `audit_events` (`old_data`, `new_data`, `metadata`) ist historische Evidenz darüber, *was war*; sie hält keine Storage-Bytes am Leben. Ebenso ausgeschlossen: `idempotency_records`, `operation_errors`, Löschticket-Snapshots, Freitext- und Link-Felder, Auth-eigene Daten und `storage.objects` selbst (Liveness fragt nach **Referenzen**, nicht danach, ob ein Objekt im Bucket existiert). Das betrifft ausschließlich die Anhang-Liveness — Audit-Daten werden dadurch **nicht** gelöscht oder verändert ([`13`](13-crm-audit-retention.md)).
- **Eine neue Spalte, die einen Objektschlüssel oder eine Storage-URL tragen kann, muss explizit registriert oder explizit ausgeschlossen werden.** Die Verifikationssuite hält dafür einen Katalog-Vollständigkeitswächter; eine nicht klassifizierte Spalte ist ein Befund ([`03`](03-data-model-guardrails.md) §1.8).

**Security-Contract:**

| Function | Modus | Owner | Settings | API-`EXECUTE` |
|---|---|---|---|---|
| `nora_private.attachment_storage_key_liveness(text)` — Resolver | `SECURITY DEFINER`, `STABLE` | `postgres` | `search_path = ''`, `row_security = off` | keine (`public`, `anon`, `authenticated`, `service_role` widerrufen) |
| `nora_private.attachment_url_liveness(text, text)` — URL-Helfer (seit S3B zusätzlich die `src`-Prüfung der Anhang-Grammatik, Abschnitt 6.11) | `SECURITY INVOKER`, `STABLE`, liest keine Tabelle | `postgres` | `search_path = ''` | keine |
| `nora_private.attachment_file_value_liveness(jsonb, text)` — Dateiwert-Helfer | `SECURITY INVOKER`, `STABLE`, liest keine Tabelle | `postgres` | `search_path = ''` | keine |

- **`SECURITY DEFINER` dient der vollständigen Sichtbarkeit, nicht einem Privileg:** der Resolver muss jede Zeile jeder registrierten Tabelle unabhängig von RLS und Aufrufer sehen — ein Resolver, der weniger Zeilen sieht, meldet ein falsches `dead`.
- **`row_security = off` macht diese Sichtbarkeit fail-closed:** würde künftige Drift (`FORCE ROW LEVEL SECURITY`, Owner-Wechsel, verlorenes `BYPASSRLS`) eine Policy auf eine registrierte Tabelle wirken lassen, wirft PostgreSQL `42501`, statt still weniger Zeilen zu liefern. Ein gefilterter Read wäre ein falsches `dead`; ein Fehler ist kein Urteil.
...
**Liveness → Warteschlange** (`inspect` ruft den Resolver genau einmal unter gültiger Lease). Seit S3A in fester Reihenfolge: Schlüssel des Auftrags lesen (ohne Sperre, autorisiert nichts) → Schlüsselsperre (Abschnitt 6.10) → lease-geschütztes `FOR UPDATE` der Warteschlangenzeile → Resolver. Abbildung, Lease- und Fehlervertrag sind unverändert; ein unbekannter Auftrag oder ein Schlüssel-/Lease-Widerspruch nach der Sperre ist `NORA_ATTACHMENT_LEASE_LOST`. Claim, Fail, Recovery und Konstanten hat S3A nicht verändert.
...
**Grenze von S2A2.2:** kein Worker, kein Aufrufer, kein Ack, kein Pfad zu `done`, kein Storage-, Netzwerk-, `pg_net`- oder Edge-Aufruf, kein neuer Warteschlangenzustand, keine Constraint- oder Spaltenänderung. S2A2.2 schließt weder das Re-Referenzierungs-Rennen noch den verwandten Verlust eines Vorhabens zwischen Erfassung und Inspektion (LOW-1). *Seit 2026-09-19* schließt S3A beides für zeilenbasierte `public.attachments`-Verweise (Abschnitt 6.10); die Absicherung externer Seiteneffekte bleibt ein S2B-Gate ([`17`](17-known-issues-and-planned-waves.md) H.1).

Regressionsprobe und operative Schritte: [`21`](21-agent-runbooks.md) Sektion 4.

### 6.10 Serialisierung je Objektschlüssel und Referenz-Zulassung (S3A)

Stand seit W8-C S3A (`PRODUCTION VERIFIED` 2026-09-19, Migration `20260919120000_nora_attachment_reference_serialization`). **Sechste Fläche, sechster Contract:** wie Referenzschreibung auf `public.attachments` (6.6), Erfassung (6.7) und Inspektion (6.9) für **denselben** Objektschlüssel koordiniert werden. Das ist ein **Integritäts- und Sicherheitsmechanismus, keine Performance-Sperre**: ohne ihn kann ein Löschvorhaben verloren gehen, oder ein künftiger Worker bekommt ein `dead` für einen Schlüssel, der gerade wieder referenziert wird.

> **S3A ist der Mechanismus, nicht der Schreiber.** Es wurde ausgeliefert, solange `public.attachments` und die Warteschlange leer waren, und sichert die Referenzfläche ab, ohne sie zu befüllen. Den Schreiber liefert seit S3B die Projektion aus den Notiz-JSON-Arrays (Abschnitt 6.11): jede Zeile, die sie einfügt oder löscht, läuft durch Zulassung und Erfassung dieses Abschnitts. Einen Worker und eine physische Löschung gibt es weiterhin nicht (S2B).

**Der eine Koordinationspunkt.** `nora_private.lock_attachment_storage_key(text)` nimmt als einzige Stelle die Schlüsselsperre `pg_advisory_xact_lock(hashtext('nora_attachment_storage_key'), hashtext(storage_key))`: transaktionsgebunden, frei erst bei `COMMIT`/`ROLLBACK`; gleicher Schlüssel → gleiche Sperre, verschiedene Schlüssel laufen parallel. Eine Hash-Kollision kann nur über-serialisieren (oder ein Warten in einen erkannten Deadlock verwandeln), nie zwei Operationen auf demselben Schlüssel überlappen lassen. Ein ungültiger Schlüssel (`NULL`, leer, länger als 512 Zeichen) wirft `22023` `NORA_ATTACHMENT_INVALID_ARGUMENT`.

| Teilnehmer | Ort | Reihenfolge in seiner Transaktion |
|---|---|---|
| Referenz-Zulassung | `AFTER INSERT`-Zeilentrigger auf `public.attachments` → `nora_private.guard_attachment_reference_admission()` | neue Zeile / Unique-Eintrag → Schlüsselsperre → Warteschlange lesen |
| Erfassung (S2A1) | `AFTER DELETE`-Zeilentrigger → `nora_private.enqueue_attachment_storage_deletion()` | gelöschte Zeile → Schlüsselsperre → Vorhaben einfügen |
| Inspektion (S2A2.2) | `nora_private.attachment_deletion_inspect(bigint, text)` | Schlüssel lesen → Schlüsselsperre → lease-geschütztes `FOR UPDATE` der Warteschlangenzeile → Resolver |

**Verbindliche Lock-Reihenfolge:** Anhangzeile / Unique-Index-Eintrag → Schlüsselsperre → Warteschlangenzeile. `claim_next`, `fail` und die Stale-Recovery warten nie auf eine Schlüsselsperre. Wer die Schlüsselsperre **vor** einem `INSERT`/`DELETE` auf `public.attachments` nimmt, kehrt die Reihenfolge gegenüber der Erfassung um und erzeugt Deadlocks. Die allgemeine Regel steht in [`03`](03-data-model-guardrails.md) §3.3.

**`READ COMMITTED` ist Teil des Vertrags, nicht nur Deployment-Kontext.** Nach dem Warten auf die Sperre muss die **nächste** Anweisung sehen, was der vorherige Halter committet hat; unter `REPEATABLE READ` / `SERIALIZABLE` stammte der Snapshot aus der Zeit vor dem Warten, und die Prüfung nach der Sperre sähe den konkurrierenden Commit nicht. Der Helfer verweigert deshalb jede andere Isolationsstufe mit `55000` `NORA_ATTACHMENT_READ_COMMITTED_REQUIRED` — und mit ihm Zulassung und Erfassung. Folge: auch ein Notiz-, Kontakt- oder Kunden-`DELETE`, dessen `CASCADE` eine Anhangzeile trifft, scheitert unter einer höheren Isolationsstufe fail-closed — und seit S3B ebenso jeder Notiz-Schreibvorgang, der das Anhang-Array ändert (Abschnitt 6.11); Schreibvorgänge ohne Anhangänderung sind nicht betroffen. Betriebsvoraussetzung: Datenbank- und Sitzungsdefault `READ COMMITTED` und **kein** abweichender Isolations-Override auf `postgres`, `authenticator`, `authenticated` oder `service_role`. Production erfüllt das (read-only vor dem Apply bestätigt, keine Konfigurationsänderung). Wer diese Defaults ändert oder einen Schreibpfad mit höherer Isolation baut, der Anhangzeilen berührt, bricht Anhang-Schreibvorgänge — oder entwirft das Protokoll neu.

**Referenz-Zulassung (Invariante I1).** Eine neue Zeile für Schlüssel K committet nicht, solange K ein **aktives** Löschvorhaben (`pending`, `claimed` — mit gültiger oder abgelaufener Lease —, `failed_retryable`) oder einen **`done`**-Grabstein hat:

- Abgewiesen wird das **ganze** `INSERT` mit `55000`, `DETAIL` `NORA_ATTACHMENT_STORAGE_KEY_PENDING_DELETION`. Die Meldung nennt keinen Schlüssel.
- `skipped_live`, `failed_terminal` und „keine Zeile" lassen zu — historische Terminalzustände blockieren nicht.
- Der Auftrag wird dabei **nie** verändert: kein Zurückziehen, kein Abbruch, kein neuer Zustand.
- `SECURITY DEFINER` (der einfügende Aufrufer hält auf der Warteschlange nichts), `search_path = ''`, `row_security = off` — RLS-Drift auf der Warteschlange wirft, statt ein Vorhaben zu verbergen; ein verborgenes Vorhaben wäre eine falsche Zulassung.
- **Oberflächenabbildung seit S3B.** Der Code steht in `NORA_ERROR_CODES` (Kategorie Konflikt) und hat eine deutsche Meldung, weil die Projektion ihn über einen Notiz-Schreibvorgang erreichbar macht — typisch ein veraltetes Formular, das eine inzwischen entfernte Datei erneut anhängt. Der ganze Notiz-Schreibvorgang wird zurückgerollt; Wiederholen hilft nicht ([`03`](03-data-model-guardrails.md) §6). *Historisch:* S3A selbst hatte bewusst keine Abbildung, weil es keinen Aufrufer gab.

**Unveränderlicher `storage_key`** und **kein direkter API-Schreibpfad**: Abschnitt 6.6.

**Security-Contract:** die drei neuen Functions (`lock_attachment_storage_key`, `guard_attachment_reference_admission`, `guard_attachment_storage_key_immutable`) liegen in `nora_private`, Owner `postgres`, `search_path = ''`, `EXECUTE` für `PUBLIC`, `anon`, `authenticated` und `service_role` widerrufen. Der Sperrhelfer und der Unveränderlichkeitswächter sind `SECURITY INVOKER`, der Zulassungswächter `SECURITY DEFINER` mit `row_security = off`. Die geänderte Erfassung bleibt `SECURITY DEFINER` (6.7), die geänderte Inspektion `SECURITY INVOKER` mit `row_security = off` (6.9) — Signaturen, Settings und ACL beider unverändert. Kein HTTP-, `pg_net`-, Storage- oder Edge-Aufruf, kein neuer Warteschlangenzustand, keine Spalten-, Constraint-, Index- oder FK-Änderung, kein neues `service_role`-Recht.

**Was S3A schließt — und was nicht:**

- **Geschlossen** für zeilenbasierte `public.attachments`-Verweise: LOW-1 (Schlüssel `dead`, Auftrag `skipped_live`, kein aktives Vorhaben — verwaistes Objekt) und das Re-Referenzierungs-Rennen (Inspektion liefert `dead`, während gleichzeitig eine neue Referenz committet).
- **Nicht geschlossen:** Verweise außerhalb von `public.attachments` nehmen die Sperre nicht — Logos, Avatare, Konfiguration. Die Notiz-Arrays nehmen seit S3B über die Projektion teil, und seit dem S4-Backfill gibt es zu ihnen auch für den Bestand Zeilen; ein über Quellen hinweg geteilter, gezielt gebauter Schlüssel bleibt ein Carry-forward für **S2B**. Die Datenbanksperre lebt nur so lange wie eine Transaktion und macht einen **externen** Storage-`DELETE` nicht sicher (S2B). Ein Upload, der vor einem später scheiternden DB-Schreibvorgang in den Bucket geht, kann weiterhin verwaisen.

> **S2B-Designgate — `claim_next()` und `inspect()` nie in derselben Transaktion.** Ein Aufrufer, der beides in **einer** Transaktion ausführt, kehrt die Lock-Reihenfolge um: der Claim hält bereits die Warteschlangenzeile und wartet dann in der Inspektion auf die Schlüsselsperre, während eine gleichzeitige Erfassung desselben Schlüssels die Schlüsselsperre hält und beim Einfügen auf die Transaktion des Claims wartet. PostgreSQL bricht das als Deadlock `40P01` ab (lokal reproduziert). Heute gibt es keinen solchen Aufrufer, es geht kein Vorhaben verloren, und die S3A-Invarianten gelten weiter — der Befund ist deshalb kein S3A-Defekt, wohl aber eine **verbindliche Grenze für S2B**: Claim und Inspektion laufen in getrennten Transaktionen, es sei denn, S2B entwirft die Lock-Reihenfolge bewusst neu **und** beweist sie nebenläufig ([`17`](17-known-issues-and-planned-waves.md) H.1).

Regressionsprobe und operative Schritte: [`21`](21-agent-runbooks.md) Sektion 4.

### 6.11 Projektion der Notiz-Anhänge in `public.attachments` (S3B)

Stand seit W8-C S3B (`PRODUCTION VERIFIED` 2026-09-19, Migration `20260919180000_nora_attachment_note_projection`). **Siebte Fläche, siebter Contract:** wie die Anhang-Arrays der Notizen (`contact_notes.attachments` / `deal_notes.attachments`) in `public.attachments` (6.6) abgebildet werden — und mit welcher Autorität.

> **Projektions-Owner ist die Datenbank, nicht die Anwendung.** Es gibt keinen Dual-Write: die Anwendung schreibt weiterhin nur das Notiz-JSON, und Trigger gleichen die Zeilen dieser Notiz **in derselben Transaktion** ab. Scheitert die Projektion, scheitert der ganze Notiz-Schreibvorgang — das JSON committet nie ohne seine Projektion. S3B ist **kein** Backfill, keine Leseseite und kein Löschpfad.

**Auslöser.** Vier `AFTER`-Zeilentrigger, je zwei auf `public.contact_notes` und `public.deal_notes`, rufen `nora_private.project_note_attachments()`:

- `INSERT` einer Notiz mit nicht-leerem Anhang-Array.
- `UPDATE`, wenn sich das Anhang-Array ändert (`NULL` und `{}` gelten als gleich). Eine reine Textänderung löst **nichts** aus. Bewusst **ohne** `UPDATE OF`-Spaltenliste: auch ein Array, das ein `BEFORE`-Trigger umschreibt, wird projiziert.
- **Kein** `DELETE`-Trigger. Das Löschen einer Notiz läuft über den FK-`CASCADE` → Zeilen-`DELETE` in `public.attachments` → Erfassung (6.7, 6.10).

**Abgleich als minimale Differenz.** `D` ist die validierte Sollmenge aus dem **neuen** Notiz-JSON, `E` die Zeilen, die der Notiz **jetzt tatsächlich** gehören — aus der Tabelle gelesen, nie aus dem alten JSON abgeleitet. Vergleichsschlüssel ist der `storage_key`.

| Menge | Wirkung |
|---|---|
| `ADD = D − E` | `INSERT` — läuft durch die Referenz-Zulassung (6.10), in deterministischer Schlüsselreihenfolge |
| `KEEP = D ∩ E` | **kein** Schreibvorgang. Titel und Typ müssen unverändert sein, sonst wird der Schreibvorgang abgewiesen — Zeilen sind unveränderlich (6.6), es gibt kein stilles Metadaten-`UPDATE` |
| `REMOVE = E − D` | `DELETE` — läuft durch die Erfassung (6.7) |

Reihenfolge: validieren → `KEEP` prüfen → `REMOVE` → `ADD`. **Minimal ist eine Korrektheitsregel, nicht nur Effizienz:** ein „alles löschen, alles neu einfügen" erfasste für jeden *behaltenen* Schlüssel ein Löschvorhaben, und das Wiedereinfügen desselben Schlüssels scheiterte in derselben Transaktion an der Zulassung. Eine geänderte Legacy-Reihenfolge oder ein geändertes `src` bei gleichem Schlüssel schreibt deshalb keine bestehende Zeile um. Seit S6-A erhält nur `ADD` einen neuen `ordinal` oberhalb des höchsten nach `REMOVE` verbleibenden Werts (`v_base + element_no`); `KEEP` behält seinen Wert, `REMOVE` hinterlässt gegebenenfalls eine Lücke. Die Legacy-Reihenfolge bleibt bis S6-B1/B2 die Laufzeitautorität. Die Projektion nimmt die Schlüsselsperre nie selbst und ruft weder Claim, Inspektion, Fail noch den Resolver auf; die Sperre entsteht in den bestehenden Triggern auf `public.attachments`. Lock-Reihenfolge: [`03`](03-data-model-guardrails.md) §3.3.

**Grammatik v1 — die eine Stelle, die entscheidet, was ein Nora-Anhangverweis ist** (`nora_private.note_attachment_reference_rows(jsonb[])`; validiert wird nur das **neue** Array, ein unsauberes altes blockiert ein gültiges neues nie):

- Jedes Element ist ein JSON-Objekt in einem eindimensionalen Array.
- `path` → `storage_key`, **wörtlich**, nie getrimmt, nie aus `src` abgeleitet: Zeichenvorrat `^[A-Za-z0-9._-]+$`, höchstens 512 Zeichen, nicht `.` oder `..`.
- `title` → `file_name` und `type` → `mime_type`: jeweils nicht-leerer String, höchstens 255 Zeichen. `byte_size` bleibt `NULL`; `rawFile` und alle übrigen Felder werden ignoriert.
- `src` fehlt, ist JSON-`null` oder ist die **kanonische öffentliche Anhang-URL genau dieses `path`** auf einem allowlisteten Origin — dieselbe Prüfung wie in 6.8 (`attachment_url_liveness`), keine zweite URL-Logik. Ein Stack auf einem nicht allowlisteten Origin kann deshalb keine Anhänge mit `src` schreiben ([`17`](17-known-issues-and-planned-waves.md) H.1, E2E-Origin).
- Derselbe `path` zweimal in **einer** Notiz wird abgewiesen — kein Dedupe, kein „erster/letzter gewinnt". Über Notizen hinweg bleibt `UNIQUE(storage_key)` maßgeblich (`23505`).
- **Ein Element ohne `path` ist kein Nora-Anhang.** Der frühere Import-Fallback (nicht ladbare fremde URL wird als Link gespeichert) ist entfallen; der Supabase-Provider weist ein solches Element schon vor dem Schreibvorgang mit demselben Code ab.
- Jede Verletzung: `22023`, `DETAIL` `NORA_ATTACHMENT_REFERENCE_INVALID`. Die Meldung nennt Elementnummer und Feld, **nie** den Schlüssel. Beide über Notiz-Schreibvorgänge erreichbaren Codes (dieser und `NORA_ATTACHMENT_STORAGE_KEY_PENDING_DELETION`, 6.10) stehen in `NORA_ERROR_CODES` und haben eine Oberflächenmeldung; die rein internen Codes (`NORA_ATTACHMENT_INVALID_ARGUMENT`, `NORA_ATTACHMENT_READ_COMMITTED_REQUIRED`) sind über die API nicht als eigener Fachfehler vorgesehen und bewusst **nicht** registriert.

**Security-Contract (drei Functions, alle in `nora_private`):**

| Function | Modus | Settings |
|---|---|---|
| `note_attachment_reference_rows(jsonb[])` — Grammatik/Validator | `SECURITY INVOKER`, `STABLE`, liest keine Tabelle | `search_path = ''` |
| `reconcile_note_attachments(bigint, bigint, jsonb[])` — Abgleich-Kern für **eine** Notiz | `SECURITY INVOKER`, `VOLATILE` | `search_path = ''`, `row_security = off` |
| `project_note_attachments()` — Trigger-Dispatcher | `SECURITY DEFINER` | `search_path = ''`, `row_security = off` |

- **Alle drei:** Owner `postgres`, ACL nur `postgres`; `EXECUTE` für `PUBLIC`, `anon`, `authenticated` **und** `service_role` widerrufen. Kein API-Aufrufpfad (6.1).
- **`SECURITY DEFINER` ist erforderlich, nicht stilistisch:** wer die Notiz schreibt, hält auf `public.attachments` nur `SELECT` (6.6) und kein `EXECUTE` auf den Kern. Die Autorität ist eng: der Besitzer wird **ausschließlich** aus der auslösenden Tabelle und `NEW.id` abgeleitet, nie aus einem Aufruferwert; geschrieben werden nur Zeilen genau dieser Notiz.
- **`row_security = off` ist fail-closed:** wirkte durch Drift eine Policy auf `public.attachments`, würfe PostgreSQL `42501`, statt Zeilen vor `E` zu verbergen — ein verborgenes `E` ergäbe ein falsches `ADD` oder ein ausgelassenes `REMOVE`.
- **Wer projiziert, entscheidet die Notiz-Autorisierung:** RLS und Grants der Notiztabellen sind unverändert. Schreibt eine berechtigte Rolle die Notiz, projiziert die Datenbank; wer die Notiz nicht schreiben darf, löst nichts aus. Die Zugriffsmatrix aus 6.6 gilt unverändert — direkte Schreibvorgänge auf `public.attachments` bleiben für alle API-Rollen verweigert.
- **Keine neue Fähigkeit:** keine Grant-Änderung, kein `service_role`-Recht auf Anhangtabelle, Warteschlange oder Functions, kein HTTP-, `pg_net`-, Storage- oder Edge-Aufruf, kein dynamisches SQL, kein `exception`-Handler, kein Worker, kein Pfad zu `done`, keine physische Löschung.
- **Isolationsstufe:** Anhang-ändernde Notiz-Schreibvorgänge erben die `READ COMMITTED`-Voraussetzung aus 6.10; der Kern ist `VOLATILE`, damit eine Anweisung, die auf die Notizzeile gewartet hat, die committeten Zeilen des Vorgängers sieht.

**First-Touch — der Mechanismus für jede unprojizierte Notiz.** Eine noch nie projizierte Notiz bleibt unprojiziert, bis ihr Anhang-Array erstmals geändert wird; weil `E` die **tatsächlichen** Zeilen sind (dann: keine), wird dabei der **gesamte** aktuelle Anhangstand der Notiz abgeglichen, nie nur die JSON-Differenz. Jede Notiz ist damit entweder unprojiziert oder exakt projiziert. Das ist opportunistischer Abgleich bei Berührung — für sich genommen **kein** Backfill.
...
**Was die Relation verbürgt — und was nicht.** Verbürgt sind Mitgliedschaft, `storage_key`, der Notiz-Besitz über den Fremdschlüssel sowie Titel (`file_name`) und Typ (`mime_type`). **S5 nutzt den seit S6-A vorhandenen `ordinal` noch nicht**: die angezeigte Array-Reihenfolge, `src`, `rawFile` und geduldete Legacy-Felder stammen bis S6-B1/B2 weiterhin aus dem Notiz-JSON. S5 verifiziert die Legacy-Nutzlast, rekonstruiert sie nicht; die Reihenfolge wird weiterhin bewusst **nicht** verglichen.
