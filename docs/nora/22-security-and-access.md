...
- **Kein direkter API-Schreibpfad — bewusst, nicht unfertig.** Ein direktes Zeilen-`DELETE` ließe die Tabelle von den Notiz-JSON-Arrays abweichen und erfasste ein Löschvorhaben für einen Schlüssel, der im JSON noch lebt; ein direktes `INSERT` schüfe Verweise, die das JSON nicht kennt. Der künftige Schreiber ist die Datenbank selbst (S3B), nicht der Client. FK-`CASCADE`s sind davon unberührt: referenzielle Aktionen laufen als Tabellen-Owner.
...
- **`CASCADE` löscht ausschließlich die Metadatenzeile, niemals das Storage-Objekt.** Keiner der Schnitte S1, S2A1, S2A2.1, S2A2.2 und S3A enthält einen physischen Löschpfad; verwaiste Objekte bleiben liegen (akzeptierte Einschränkung seit W8-B, [`17`](17-known-issues-and-planned-waves.md) H.1). Nie als „die Datei wird gelöscht" formulieren. Seit S2A1 hängt an derselben Zeilenlöschung ein `AFTER DELETE`-Trigger, der ein Lösch**vorhaben** festhält — Abschnitt 6.7.
...
- **Wer eine Erfassung auslöst:** jedes Zeilen-`DELETE` auf `public.attachments`. Seit S3A hält keine API-Rolle mehr ein direktes `DELETE` (Abschnitt 6.6) — heute entsteht ein Vorhaben nur über FK-`CASCADE` einer Notiz-, Kontakt- oder Kundenlöschung bzw. über Owner-Pfade, und da die Tabelle leer ist, in Production gar nicht. *Historisch* hielt Büro/Admin seit S1 ein direktes `DELETE`; S2A1 hat daran nichts geändert, S3A hat es entzogen. Die Policies aus 6.5 sind unberührt.
...
> **`dead` ist eine Beobachtung, niemals eine Löscherlaubnis.** Das Ergebnis beschreibt, was **ein** Statement-Snapshot gesehen hat. Der Ausführungsvertrag (S2A2.2, Abschnitt 6.9) persistiert `dead` deshalb nicht, sondern gibt es nur an den Lease-Halter zurück. Seit S3A ruft die Inspektion den Resolver erst **unter der Schlüsselsperre** (Abschnitt 6.10): jede Löschung oder Zulassung einer `public.attachments`-Zeile für diesen Schlüssel ist dann entweder committet und sichtbar oder noch nicht begonnen. Das schließt das Re-Referenzierungs-Rennen für diese **zeilenbasierten** Verweise — nicht für Schreibvorgänge auf die übrigen registrierten Flächen (Legacy-JSON, Logos, Avatare, Konfiguration), die an der Sperre nicht teilnehmen, und nicht für externe Seiteneffekte. Physisch löschen darf weiterhin erst S2B ([`17`](17-known-issues-and-planned-waves.md) H.1).
...
| `nora_private.attachment_url_liveness(text, text)` — URL-Helfer | `SECURITY INVOKER`, `STABLE`, liest keine Tabelle | `postgres` | `search_path = ''` | keine |
...
> **S3A ist Mechanismus vor der ersten Zeile.** `public.attachments` und die Warteschlange sind in Production leer. Es gibt **keine** Projektion aus den Notiz-JSON-Arrays (S3B), keinen Worker und keine physische Löschung (S2B). S3A sichert die Referenzfläche ab, bevor sie befüllt wird — es befüllt sie nicht.
...
- **Keine Oberflächenabbildung.** Der Code steht nicht in `NORA_ERROR_CODES`, und kein Nutzer sieht dafür eine eigene Meldung — es gibt keinen Aufrufer, der Zeilen schreibt. Die Abbildung gehört zu dem Schnitt, der erstmals schreibt ([`03`](03-data-model-guardrails.md) §6).
...
- **Nicht geschlossen:** Schreibvorgänge auf die übrigen registrierten Flächen (Legacy-JSON bis S3B, Logos, Avatare, Konfiguration) nehmen die Sperre nicht; ein über Quellen hinweg geteilter, gezielt gebauter Schlüssel bleibt ein Carry-forward für S2B/S4. Die Datenbanksperre lebt nur so lange wie eine Transaktion und macht einen **externen** Storage-`DELETE` nicht sicher (S2B). Ein Upload, der vor einem später scheiternden DB-Schreibvorgang in den Bucket geht, kann weiterhin verwaisen.
