...
Stand: 2026-09-19 · Status: **CURRENT** · Load-Klasse: **CONDITIONAL CURRENT CONTRACT**
...
- **Nicht geschlossen:** Verweise außerhalb von `public.attachments` nehmen die Sperre nicht — Logos, Avatare, Konfiguration sowie das JSON von Bestandsnotizen, die noch nie projiziert wurden (bis S4; Anhang-ändernde Notiz-Schreibvorgänge nehmen seit S3B über die Projektion teil); ein über Quellen hinweg geteilter, gezielt gebauter Schlüssel bleibt ein Carry-forward für S2B/S4. Die Datenbanksperre lebt nur so lange wie eine Transaktion und macht einen **externen** Storage-`DELETE` nicht sicher (S2B). Ein Upload, der vor einem später scheiternden DB-Schreibvorgang in den Bucket geht, kann weiterhin verwaisen.
...
**Bestandsnotizen bis S4 — First-Touch, kein Backfill.** S3B hat keine historische Notiz projiziert. Eine Bestandsnotiz bleibt unprojiziert, bis ihr Anhang-Array erstmals geändert wird; weil `E` die **tatsächlichen** Zeilen sind (dann: keine), wird dabei der **gesamte** aktuelle Anhangstand der Notiz abgeglichen, nie nur die JSON-Differenz. Jede Notiz ist damit entweder unprojiziert oder exakt projiziert. Das ist opportunistischer Abgleich bei Berührung — **kein** abgeschlossener Backfill und keine globale Parität. Bekannte, akzeptierte Lücke dieses Fensters: ein Anhang, der aus einer Bestandsnotiz entfernt wird, **bevor** sie je projiziert wurde, hat keine Zeile — es entsteht kein Löschvorhaben. Gegenüber dem Stand vor S3B ist das keine Verschlechterung ([`17`](17-known-issues-and-planned-waves.md) H.1).

**Grenze zu S4.** Der Backfill verwendet **denselben** Kern `nora_private.reconcile_note_attachments(...)` unter einer Sperre der Notizzeile — der Kern setzt voraus, dass der Aufrufer die Notizzeile hält (das auslösende `UPDATE` tut das). S4 führt **keinen** zweiten Abgleich-Algorithmus und keine zweite Grammatik ein. Die Umschaltung der Leseseite (S5) bleibt blockiert, bis S4 `PRODUCTION VERIFIED` ist.
