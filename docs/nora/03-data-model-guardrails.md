...
- **Seit W8-C S5 ist die Relation die Vertrauensquelle des Anhang-Lesepfads — nicht die Anhang-Nutzlast** ([`22`](22-security-and-access.md) Abschnitt 6.12):
  - **Die Autorität ist geteilt, und zwar dauerhaft bis S6.** Die Zeilen verbürgen Mitgliedschaft, `storage_key`, Notiz-Besitz (FK), Titel und Typ. Array-Reihenfolge, `src`, `rawFile` und geduldete Legacy-Felder bleiben allein im Notiz-JSON. **Nie eine Anhangliste aus `public.attachments` rekonstruieren** — die Zeilen tragen diese Felder nicht, und was sie nicht tragen, darf nicht erfunden werden. Die Reihenfolge wird deshalb bewusst nicht verglichen.
