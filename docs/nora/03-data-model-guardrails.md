...
Stand: 2026-09-20 · Load-Klasse: **ALWAYS**
...
### 1.8 Anhang-Metadaten: ein Besitzer, eine datenbankeigene Projektion, `CASCADE` löscht keine Datei, Referenzspalten sind registriert

Seit W8-C S1 existiert `public.attachments`; seit W8-C S3B ist sie die **datenbankeigene Projektion** der JSON-Arrays `contact_notes.attachments` / `deal_notes.attachments`. Die Anwendung schreibt und liest weiterhin nur diese Arrays — das Legacy-JSON ist **nicht** abgelöst (S6). Die Bestandsnotizen sind seit W8-C S4 nachprojiziert ([`17`](17-known-issues-and-planned-waves.md) H.1); eine Leseseite auf der Tabelle gibt es deswegen nicht (S5). Vollständiger Contract (Spalten, RLS, Grants, Grammatik, Begründungen): [`22`](22-security-and-access.md) Abschnitte 6.6 und 6.11 — hier stehen nur die Invarianten, die beim Datenmodellieren zählen:
