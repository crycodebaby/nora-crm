...
- **Baustellenadressen** sind noch keine eigenen Objekte. Der vorbereitete RC speichert den Einsatzort ausschließlich in `deals.site_*`, unabhängig von der Kundenanschrift. Vorschläge aus der Kundenanschrift nur bei Neuanlage und bis zum ersten manuellen Site-Eingriff; Leeren bleibt als `NULL` leer, Edit synchronisiert nie. Keine unsichtbaren Quick-Capture-Writes, kein Backfill. Entscheidung: `06` „Eigenständige Einsatzadresse am Vorgang“. *(Falle 2)*
...
### 1.8 Anhang-Metadaten: ein Besitzer, eine datenbankeigene Projektion, ein verifiziertes Read Model, `CASCADE` löscht keine Datei, Referenzspalten sind registriert

Seit W8-C S1 existiert `public.attachments`; seit W8-C S3B ist sie die **datenbankeigene Projektion** der JSON-Arrays `contact_notes.attachments` / `deal_notes.attachments`. Die Anwendung **schreibt** weiterhin nur diese Arrays — das Legacy-JSON ist **nicht** abgelöst (S6-B1/B2). Die Bestandsnotizen sind seit W8-C S4 nachprojiziert ([`17`](17-known-issues-and-planned-waves.md) H.1), und seit W8-C S5 **liest** die Anwendung die Tabelle als Vertrauensquelle ihres Anhang-Read-Models (vorletzter Punkt). Vollständiger Contract (Spalten, RLS, Grants, Grammatik, Lese-Gate, Begründungen): [`22`](22-security-and-access.md) Abschnitte 6.6, 6.11 und 6.12 — hier stehen nur die Invarianten, die beim Datenmodellieren zählen:
...
- **Seit W8-C S3A ist `public.attachments` eine datenbankeigene Schreibfläche mit Referenz-Zulassung** ([`22`](22-security-and-access.md) Abschnitte 6.6 und 6.10):
  - Keine API-Rolle schreibt direkt; `authenticated` liest nur. Der einzige Schreiber ist die Projektion aus den Notiz-JSON-Arrays (nächster Punkt).
  - Ein `INSERT` für einen Schlüssel mit aktivem Löschvorhaben (`pending`, `claimed`, `failed_retryable`) oder mit `done` scheitert (`NORA_ATTACHMENT_STORAGE_KEY_PENDING_DELETION`); `skipped_live` und `failed_terminal` blockieren nicht.
  - `storage_key` ist nach dem Anlegen unveränderlich (`NORA_ATTACHMENT_STORAGE_KEY_IMMUTABLE`). Einen Verweis auf ein anderes Objekt umzuhängen heißt: alte Zeile löschen, neue anlegen — nie `UPDATE … SET storage_key`.
  - Einfügen und Löschen einer Anhangzeile verlangen `READ COMMITTED` und nehmen die Schlüsselsperre aus §3.3; unter `REPEATABLE READ` / `SERIALIZABLE` scheitern sie fail-closed — auch als `CASCADE` einer Notiz-, Kontakt- oder Kundenlöschung und, seit S3B, als Folge eines Notiz-Schreibvorgangs, der das Anhang-Array ändert.
- **Seit W8-C S3B projiziert die Datenbank das Anhang-Array einer Notiz selbst — in derselben Transaktion wie den Notiz-Schreibvorgang** ([`22`](22-security-and-access.md) Abschnitt 6.11):
  - **Projektions-Owner sind Trigger, kein Dual-Write.** Kein Anwendungscode, kein Import und keine Automatisierung schreibt `public.attachments` „zusätzlich" zum JSON; wer Anhänge ändert, ändert das Notiz-Array. Scheitert die Projektion, scheitert der ganze Notiz-Schreibvorgang — JSON und Zeilen committen nie in verschiedenen Zuständen.
  - **Identität ist der `storage_key` (aus `path`)** — nicht `src`, nicht der Titel, nicht die Position im Array. Umsortieren oder ein anderes `src` bei gleichem Schlüssel ist keine Änderung. Ein **bereits projizierter** Schlüssel, dessen Titel oder Typ vom Stand seiner Zeile abweicht, wird **abgewiesen**, nie still aktualisiert — verglichen wird gegen die tatsächliche Zeile, nie gegen das alte JSON. Beim First-Touch einer nie projizierten Bestandsnotiz (letzter Punkt) gibt es keine Zeile: ihre Schlüssel werden mit Titel und Typ des neuen Arrays eingefügt, nicht abgewiesen. Derselbe Schlüssel zweimal in einer Notiz wird immer abgewiesen.
  - **Abgleich als minimale Differenz** gegen die **tatsächlichen** Zeilen der Notiz: Neues einfügen, Entfallenes löschen, Unverändertes nicht anfassen. Nie „alles löschen, alles neu einfügen": jedes unnötige `DELETE` erfasste ein Löschvorhaben und blockierte das Wiedereinfügen desselben Schlüssels. Das ist Korrektheit, nicht Optimierung.
  - **Ein Notiz-Anhang braucht einen Objektschlüssel.** Die zentrale Grammatik (v1) verlangt `path`, Titel und Typ; ein Element ohne `path` — etwa ein Import-Verweis auf eine fremde URL — ist kein Nora-Anhang und wird nicht gespeichert. Die Grammatik steht an **genau einer** Stelle ([`22`](22-security-and-access.md) Abschnitt 6.11) und wird nirgends nachgebaut — auch der S4-Backfill hat sie aufgerufen, nicht kopiert. Verletzungen scheitern atomar mit `NORA_ATTACHMENT_REFERENCE_INVALID` (§6).
  - **First-Touch ist der Mechanismus; den Bestand hat S4 nachgezogen.** Eine nie projizierte Notiz wird beim ersten Anhang-ändernden Schreibvorgang **vollständig** abgeglichen; eine reine Textänderung projiziert nichts. Die bei S3B unberührten Bestandsnotizen hat W8-C S4 nachprojiziert; bei der Production-Verifikation am 2026-09-20 stimmten Notiz-JSON und `public.attachments` global überein. **Das ist eine verifizierte Momentaufnahme, keine vom Schema erzwungene Invariante** — es gibt keinen Constraint, der globale Parität garantiert. Weiterhin gilt deshalb: nie aus einer fehlenden Zeile schließen, eine Notiz habe keine Anhänge, und die Zeilenzahl der Tabelle nie als Invariante behandeln.
- **Seit W8-C S5 ist die Relation die Vertrauensquelle des Anhang-Lesepfads — nicht die Anhang-Nutzlast** ([`22`](22-security-and-access.md) Abschnitt 6.12):
  - **Die Autorität bleibt bis S6-B1/B2 geteilt.** Die Zeilen verbürgen Mitgliedschaft, `storage_key`, Notiz-Besitz (FK), Titel und Typ; seit S6-A tragen sie außerdem `ordinal integer NOT NULL` ohne Default und die nicht aufschiebbare Eindeutigkeitsregel `UNIQUE NULLS NOT DISTINCT (contact_note_id, deal_note_id, ordinal)`. Das ist eine relationale Ordnungsgrundlage, noch keine Lese- oder Schreibautorität der Anwendung. `src`, `rawFile`, geduldete Legacy-Felder und die angezeigte Reihenfolge kommen weiterhin aus dem Notiz-JSON. **Nie eine vollständige Anhangliste aus `public.attachments` rekonstruieren** — die Zeilen tragen die übrige Nutzlast nicht. S5 vergleicht die Reihenfolge weiterhin nicht.
  - **Verifiziert leer (`[]`) ist nicht dasselbe wie nicht verbürgt (`null`).** Die beiden werden nirgends ineinander normalisiert. Wer sie gleichsetzt, macht aus einer degradierten Notiz eine scheinbar anhanglose — dieselbe Verwechslung, gegen die das Gate gebaut ist.
  - **Fail closed, in beide Richtungen.** Ein Notiz-Lesevorgang **ohne** relationale Evidenz gilt als *nicht verbürgt*, nie als *in Ordnung*; und was nicht verbürgt ist, ist lesbar, aber **anhangseitig nicht beschreibbar**. Nicht-anhangbezogene Änderungen an derselben Notiz bleiben möglich. Ein degradierter Zustand erwirbt nie Schreibautorität, und nichts repariert ihn automatisch.
  - **Projektions-Owner bleibt die Datenbank (S3B).** S5 hat daran nichts geändert: es gibt weiterhin keinen Dual-Write und keinen direkten API-Schreibpfad auf `public.attachments`. Das Lesen nutzt ausschließlich das vorhandene `SELECT`-Recht; S5 bringt keine Migration und keine neue Berechtigung.
...
**Zweite Instanz: Anhang-Objektschlüssel (W8-C S3A).** Referenz-Zulassung, Erfassung eines Löschvorhabens und Inspektion eines Löschauftrags koordinieren sich je `storage_key` über **eine** transaktionsgebundene Advisory-Sperre (`nora_private.lock_attachment_storage_key`, Namespace `nora_attachment_storage_key`). Auch hier liegt die Advisory-Sperre vor der konfliktträchtigen Zeilensperre, der Warteschlangenzeile; die Anhangzeile selbst ist unvermeidbar schon gehalten, weil Zulassung und Erfassung `AFTER`-Trigger sind. Die durable Lock-Reihenfolge:

> **Anhangzeile / Unique-Index-Eintrag → Schlüsselsperre → Warteschlangenzeile.** Zulassung und Erfassung sind `AFTER`-Zeilentrigger und halten die Zeile zuerst; die Inspektion hält keine Anhangzeile und nimmt die Schlüsselsperre **vor** der Warteschlangenzeile. Claim, Fail und Recovery warten nie auf eine Schlüsselsperre.

Seit W8-C S3B steht davor die **Notizzeile**: ein Anhang-ändernder Notiz-Schreibvorgang hält sie bereits, wenn die Projektion Anhangzeilen einfügt oder löscht — Notizzeile → Anhangzeile → Schlüsselsperre → Warteschlangenzeile. Die Projektion nimmt die Schlüsselsperre nie selbst. Der S4-Backfill hat sich als erster direkter Aufrufer des Abgleich-Kerns an dieselbe Reihenfolge gehalten: zuerst `FOR UPDATE` auf die Notizzeile, dann der Kern — jeder künftige Aufrufer tut dasselbe.

Daraus folgen zwei Verbote: die Schlüsselsperre **nie vor** einem `INSERT`/`DELETE` auf `public.attachments` nehmen (umgekehrte Reihenfolge gegenüber der Erfassung), und **nie** eine Warteschlangenzeile halten, bevor in derselben Transaktion die Schlüsselsperre angefordert wird — genau das täte ein `claim_next()` gefolgt von `inspect()` in **einer** Transaktion; gegen eine gleichzeitige Erfassung endet es in `40P01`. Ein künftiger Worker (S2B) führt Claim und Inspektion deshalb in getrennten Transaktionen aus, außer er entwirft und beweist eine neue Reihenfolge. Das Protokoll setzt `READ COMMITTED` voraus: erst nach dem Warten auf die Sperre muss die nächste Anweisung den Commit des vorherigen Halters sehen. Contract: [`22`](22-security-and-access.md) Abschnitt 6.10.
