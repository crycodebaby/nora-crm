...
- **Seit W8-C S3A ist `public.attachments` eine datenbankeigene Schreibfläche mit Referenz-Zulassung** ([`22`](22-security-and-access.md) Abschnitte 6.6 und 6.10):
  - Keine API-Rolle schreibt direkt; `authenticated` liest nur. Der vorgesehene Schreiber ist eine Projektion aus den Notiz-JSON-Arrays (S3B) — sie **existiert noch nicht**.
  ...
  - Einfügen und Löschen einer Anhangzeile verlangen `READ COMMITTED` und nehmen die Schlüsselsperre aus §3.3; unter `REPEATABLE READ` / `SERIALIZABLE` scheitern sie fail-closed — auch als `CASCADE` einer Notiz-, Kontakt- oder Kundenlöschung.
