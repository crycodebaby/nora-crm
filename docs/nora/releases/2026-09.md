...
## 2026-09-07 – Markierungen Identity Wave (RC, nicht released)

**Status: `RC VERIFIED` — kein Push, kein Production-Apply, keine Production-Bereinigung.**

Branch `feat/nora-markierungen-workflow`, Basis `origin/main` = `0ee6281c`.
Migration: `supabase/migrations/20260907140000_nora_tag_identity.sql`.

### Reproduzierte Ursache

Ein Browser-Test gegen den **unveränderten** Stand `0ee6281c` erzeugte den Production-Effekt exakt:

```
TypeError: record.tags is not iterable
  ← TagsListEdit.handleTagCreated: [...record.tags, tag.id]
```

Ablauf auf einem Kontakt mit `contacts.tags IS NULL`: Tag-Zeile wird geschrieben → Anhängen wirft → `TagDialog` erreicht sein `handleClose()` nie → Dialog bleibt offen, **ohne** Fehlermeldung → `finally` aktiviert „Speichern“ wieder → zweiter Klick = zweite Zeile. Kontrollfall `tags: []` funktionierte fehlerfrei — nur die NULL-Spalte war betroffen.

Passend dazu in Production (read-only, 2026-09-07): genau **zwei** Kontakte mit `tags IS NULL` (id 26, 29) und **fünf** überzählige Tag-Zeilen mit Verwendung **0**.

### Production-Bestand (read-only)

| id | Name | Farbe | Verwendung |
|---|---|---|---|
| 1 | `Kunde AE` | `#99c1de` | 6 |
| 2 | `Erfunden` | `#f0efeb` | 1 |
| 3 | `Privatperson` | `#99c1de` | 0 |
| 4 | `Privatperson` | `#99c1de` | 0 |
| 5 | `Privatperson` | `#99c1de` | 0 |
| 6 | `Privatperso` | `#99c1de` | 0 |
| 7 | `Privatperso` | `#99c1de` | 0 |

### Cleanup-Vorschau für Production (berechnet, **nicht ausgeführt**)

Mit dem Algorithmus der Migration gegen die Live-Daten gerechnet:

| Kennzahl | Wert |
|---|---|
| Zeilen gelöscht | **3** (id 4, 5 → kanonisch 3; id 7 → kanonisch 6) |
| Zeilen erhalten | **4** (id 1, 2, 3, 6) |
| Kontakte, deren `tags`-Array sich ändert | **0** |
| Kontakte `NULL` → `'{}'` | **2** (id 26, 29) |
| Namen getrimmt | 0 |
| Leere Namen | 0 |

Präflug gegen Production grün: 0 verwaiste Tag-Ids, 0 leere benutzte Namen, **0 mehrdeutige Gruppen**, 0 Kontakte mit doppelter Tag-Id, Invariante noch nicht vorhanden.

Alle sieben Kontakt→Markierung-Beziehungen bleiben unverändert (nur id 1 und 2 sind überhaupt referenziert). **„Privatperso“ (id 6) bleibt bestehen** — siehe `17-known-issues-and-planned-waves.md` G.0a.

### Migrationsnachweis (echtes Postgres 17.6)

Isolierte Datenbank `nora_tag_rc` (Schema-Dump des lokalen Stacks, damit der parallel laufende RC einer anderen Welle unberührt bleibt), befüllt mit der **exakten** Production-Fixture:

* Lauf 1: `DELETE 3`, `UPDATE 2` (NULL-Backfill), alle Endzusicherungen grün; Endstand `1 Kunde AE, 2 Erfunden, 3 Privatperson, 6 Privatperso`, alle 7 Beziehungen erhalten, 0 NULL, 12 leere Arrays.
* Lauf 2 (Idempotenz): `DELETE 0`, `UPDATE 0`, Endzusicherungen erneut grün, Datenstand identisch.
* Invariantenbeweise: exakte Dublette, Case-Variante, Whitespace-Variante und Rename-Kollision werden von `uq__tags__normalized_name` abgelehnt; leerer Name → `NORA_TAG_NAME_REQUIRED`; `'  Gewerbe  '` wird als `Gewerbe` gespeichert; `{1,1,1}` → `{1}`; explizites `tags = null` → `{}`; Löschen einer benutzten Markierung → `NORA_TAG_IN_USE`; Löschen einer unbenutzten erlaubt.
* **Nebenläufigkeit:** zwei gleichzeitige Sessions (über `pg_advisory_xact_lock` synchronisiert) legen denselben Namen an → genau **eine** Zeile committet, die Verliererin bekommt `23505`, die der Client in „existiert bereits, wird verwendet“ übersetzt.

### Test-Evidenz

`npm run typecheck`, `npm run build` und `npx eslint` grün. Vollständige Suite: **118 Dateien, 1048 Tests, 1 übersprungen — 0 Fehler**, davon neu:

| Datei | Tests |
|---|---|
| `application/commands/ensureTag.test.ts` | 20 |
| `contacts/TagsListEdit.test.tsx` | 13 |
| `tags/tagIdentity.parity.test.ts` | 16 |
| `tags/TagManagerDialog.test.tsx` | 7 |
| `contacts/BulkTagButton.test.tsx` | 4 |
| `contacts/ContactListFilter.test.tsx` | 4 |

Zwei echte Befunde kamen aus diesen Tests, nicht aus dem Review:
1. Ein `useState`-Guard gegen Doppelklicks **reicht nicht** — zwei Klicks im selben Tick sehen denselben Render. Ersetzt durch einen synchronen `useRef`-Guard.
2. Der FakeRest-Eindeutigkeitswächter war zwischen Lesen und Schreiben unterbrechbar; Tag-Schreibvorgänge werden jetzt pro Provider serialisiert, damit der Demo-Modus dasselbe Ergebnis liefert wie Postgres.

### Sichtprüfung (`npm run dev:demo`, Chrome)

Picker zeigt „Vorhandene Markierungen“ zuerst, „Neue Markierung erstellen“ und „Markierungen verwalten“ darunter. `„  Stammkunde  “` → Hinweis „Markierung „stammkunde“ existiert bereits und wird verwendet“ **vor** dem Speichern; nach Speichern Toast „… existiert bereits und wurde hinzugefügt“ und sichtbarer Chip, **keine** zweite Zeile. Neue Markierung „Privatperson“: Button zeigt während des Schreibens Spinner + „Wird gespeichert …“, Feld deaktiviert; danach Dialog zu, Chip sichtbar, Toast „… wurde erstellt und hinzugefügt“. Leerer Name → „Speichern“ deaktiviert. „Markierungen verwalten“ zeigt Verwendungszähler; der Papierkorb ist nur bei Verwendung 0 aktiv. Keine Konsolenfehler.

### Release-Runbook (offen)

1. Branch pushen, PR gegen `main`.
2. Production-Präflug erneut fahren (die vier Zähler oben) — bei „mehrdeutige Gruppen > 0“ **nicht** anwenden, sondern die Gruppe fachlich klären.
3. `20260907140000_nora_tag_identity.sql` anwenden; die Migration bricht selbständig ab, statt zu raten. Ledger-Version nach `apply_migration` prüfen.
4. Nachweis: Tag-Liste (4 Zeilen erwartet), 7 Kontakt-Beziehungen unverändert, `select count(*) from public.contacts where tags is null` = 0, `uq__tags__normalized_name` vorhanden.
5. Fast-Forward auf `origin/main`, Vercel-Deployment abwarten.
6. PO-Smoke: Markierung anlegen, gleichnamige Markierung anlegen (muss wiederverwenden), Markierung entfernen, „Markierungen verwalten“ → `Privatperso` löschen (G.0a).
