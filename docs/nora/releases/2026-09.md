...
| 2026-09-07 | User Lifecycle W6-B (kontrollierter Hard Delete) | **`PRODUCTION VERIFIED`** | Laufzeit `ffc0183a` (DB `85b1e366` + Edge/UI `8e556639`); Ledger 56 / `20260906230000`, `users`-Edge v9, Vercel READY, Live-Beweis Testkonto `sales.id = 4` | `20260906230000_nora_lifecycle_account_deletion` (LF SHA-256 `9b67b18efd3fa4c6bf5dd6d2f893efa1221561527b33b9c1fef98d5ea3980b81`) — **nicht angewendet** | `users` Quelltext geändert — **nicht deployt** (Production weiterhin v8) | lokal: SQL-Gate 35/35, realer GoTrue-HTTP-Beweis 42/42, Vitest App 984 / Functions 377, Build grün; Production nur read-only gelesen, `sales 4` unverändert |

Nicht Teil eines Releases in diesem Monat: W6-B (Hard Delete) nicht begonnen; SQL-Suiten weiterhin nicht in CI (W9).


## Security Hardening Wave 1 — Privilegien im Schema `public` (RC 2026-09-07, **nicht angewendet**)

**Status: `RC VERIFIED — NICHT AUF PRODUCTION ANGEWENDET, NICHT DEPLOYT, NICHT GEPUSHT`.**
Durable Entscheidung: `06-decision-log.md` 2026-09-07. Vertrag: `03-data-model-guardrails.md` „Privilegien im Schema `public`". Offener Punkt: `17-known-issues-and-planned-waves.md` A.1 (Status `RC`) und A.8 (neu).
