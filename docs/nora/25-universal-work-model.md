...
> **Befund, protokolliert.** `operationCatalog.ts` enthält heute **keinen** `task.*`-Eintrag; `AddTask.tsx` und `Task.tsx` schreiben über rohes `useCreate`/`useUpdate` — ohne `operation_id`, ohne Idempotenz, ohne Error-Contract-Pfad. Jede künftige Core Work Transition wäre der **erste** korrelierte Task-Schreibvorgang. `deal.assign` existiert bereits als vorbereiteter Katalogeintrag; dessen Vokabular wird wiederverwendet, kein zweites erfunden.
...
| `title` | `tasks.text` | nullable; `null` ⟺ `validity = incomplete`. Nie clientseitig ersetzt |
...
> **Benannte Kollision mit offenen Tracks.** Eine `SECURITY DEFINER` Application Query umgeht `is_active_user()` und **vergrößert** die Fläche, die die offene Security-Welle ([`17`](17-known-issues-and-planned-waves.md) A) vorfindet. W-A muss so gebaut werden, dass es sie nicht vergrößert. Ebenfalls benannt: für eine serverseitige Application Query existiert heute **kein** FakeRest-Äquivalent ([`03`](03-data-model-guardrails.md) §5) — eine reale, bewusst akzeptierte W-A-Kost. Und: `tasks_company_id_fkey ON DELETE CASCADE` löscht Arbeit bei einer Kundenlöschung still mit; kein Work-Pfad darf Aufgaben als dauerhaft annehmen.
