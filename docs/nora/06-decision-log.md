...
## 2026-09-07 – Markierungen Identity: eine logische Markierung existiert höchstens einmal

**Kontext.** In Production entstanden fünf überzählige `public.tags`-Zeilen (3× „Privatperson“, 2× „Privatperso“), alle ohne jede Verwendung. Reproduzierte Ursache: `TagsListEdit.handleTagCreated` spreizte `record.tags` ungeprüft (`[...record.tags, tag.id]`). Bei einem Kontakt, dessen Spalte `contacts.tags` **NULL** war, warf das `TypeError: record.tags is not iterable` — **nachdem** die Markierung bereits angelegt war. Der Dialog blieb offen, zeigte keinen Fehler, der Speichern-Button wurde wieder aktiv: Jeder weitere Klick erzeugte eine weitere Zeile.

**Entscheidungen.**

1. **Kanonischer Name.** Die Identität eines Markierungsnamens ist `lower(btrim(name))` (`nora_private.tag_name_key`). Umgebende Leerzeichen und Groß-/Kleinschreibung sind bedeutungslos, **alles andere nicht**: „Privatperso“ und „Privatperson“ bleiben zwei verschiedene Markierungen. Eine Tippfehler-Zusammenlegung ist eine Aussage über Bedeutung und darf nie automatisch geschehen.
2. **Die Datenbank hält die Regel, nicht die UI.** `uq__tags__normalized_name` ist die Wahrheit; der Client prüft nur aus Höflichkeit vorab. Ein zweiter Tab, ein Retry oder ein CSV-Import können deshalb keine Dublette erzeugen — der Verlierer eines Rennens löst die Gewinnerzeile auf und benutzt sie (`NORA_TAG_ALREADY_EXISTS`), statt dem Büro einen Constraint-Fehler zu zeigen.
3. **`contacts.tags` ist `not null default '{}'`** und wird per Trigger dedupliziert. Die Nullable-Spalte war die eigentliche Falle; kein Client kann sie mehr stellen.
4. **Eine autoritative Erzeugung.** `application/commands/ensureTag.ts` ist der einzige Weg von „getippter Name“ zu „Tag-Zeile“ — Einzelkontakt, Bulk-Tagging und CSV-Import benutzen ihn. Reparaturen an drei Stellen mit leicht unterschiedlicher Logik sind ausdrücklich nicht der Weg.
5. **Umbenennen auf einen belegten Namen wird abgelehnt, nie still zusammengeführt.** Zwei Markierungen zu einer zu machen ist eine bewusste fachliche Entscheidung; ein Merge-Werkzeug für **benutzte** Markierungen existiert bewusst noch nicht (siehe `17-known-issues-and-planned-waves.md`).
6. **Löschen nur bei Verwendung 0**, serverseitig durch `nora_private.guard_tag_delete()` erzwungen — sonst blieben verwaiste Tag-Ids in `contacts.tags` zurück.
7. **Jede Mutation hat einen sichtbaren Endzustand.** „Wird gespeichert …“ während des Schreibens, ein synchroner Ref-Guard gegen Doppelklicks (State reicht nicht: zwei Klicks im selben Tick sehen denselben Render), Erfolgsmeldung mit Namen, und bei Fehlern bleibt der Dialog **offen** mit deutscher Begründung.
8. **Bereinigung ist datengetrieben.** Die Migration leitet Dublettengruppen aus dem Namensschlüssel ab (keine fest verdrahteten Production-Ids), schreibt Referenzen auf die kanonische Zeile um, beweist, dass kein Datensatz eine logische Markierung verliert, und **bricht ab**, wenn eine Gruppe mehrere *benutzte* Zeilen mit verschiedenen Farben enthält.

**Begründung.** Ein reiner UI-Fix hätte denselben Zustand jederzeit wieder entstehen lassen: Vor dieser Welle gab es keine Regel, die zwei Zeilen mit demselben logischen Namen verhindert hätte. Die Invariante gehört in die Daten, die Verständlichkeit in die Oberfläche.
