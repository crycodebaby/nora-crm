...
## 2026-09-21 – W8-C S5: Die Anwendung liest Notiz-Anhänge nur noch als verifiziertes Read Model — der Legacy-Schreibvertrag bleibt bis S6 bestehen

**Status:** `PRODUCTION VERIFIED` (Laufzeit `3f9dd9edc2e7e5c73cb1dcf0184a7b17955e26d5`, **keine Migration**, Production-Verifikation 2026-09-21). Contract: [`22`](22-security-and-access.md) Abschnitt 6.12 (Lese-Gate, Schreibwächter, Darstellungsgrenze) und 6.6 (Zugriff); Invarianten: [`03`](03-data-model-guardrails.md) §1.8. Ist-Zustand: [`16`](16-current-state.md) Abschnitt „Anhänge / Storage". Offene Folgeschnitte und Gates: [`17`](17-known-issues-and-planned-waves.md) H.1. Evidenz: `releases/2026-09.md`.

**Kontext.** Nach S3B projizierte die Datenbank die Notiz-Anhänge, nach S4 stimmten Notiz-JSON und `public.attachments` global überein — aber die Anwendung las die Tabelle nicht. Die Oberfläche vertraute dem Legacy-JSON bedingungslos: ein Array-Element, für das es keine relationale Zeile gab, wurde genauso als Anhang gerendert und weitergeschrieben wie ein verbürgtes. Offen war, **was** die Tabelle auf der Leseseite eigentlich verbürgt, **wie** sich die Anwendung verhält, wenn sie es nicht kann, und wie verhindert wird, dass der Legacy-Schreibvertrag dabei versehentlich mit abgelöst wird.

**Entscheidungen.**

1. **`public.attachments` wird die Vertrauensquelle des Lesepfads — nicht die vollständige Nutzlast.** Relational verbürgt sind Mitgliedschaft, `storage_key`, der Notiz-Besitz über den Fremdschlüssel, Titel (`file_name`) und Typ (`mime_type`). **Die Autorität ist damit geteilt:** Array-Reihenfolge, `src`, `rawFile` und geduldete Legacy-/Präsentationsfelder bleiben bis S6 beim Notiz-JSON. Begründung: die Zeilen tragen diese Felder gar nicht. Sie aus der Tabelle zu rekonstruieren hieße, Nutzerdaten zu erfinden — **S5 verifiziert die Legacy-Nutzlast, es ersetzt sie nicht.** Deshalb wird auch die Reihenfolge bewusst **nicht** verglichen.
2. **Drei Lesezustände, und der unangereicherte Lesevorgang ist einer davon.** `ok` (die Zeilen verbürgen das Array), `drift` (angereichert gelesen, Zeilen und JSON stimmen nicht überein), `unverified` (der Lesevorgang trug gar keine Zeilen). Begründung: „keine Zeilen gesehen" und „geprüft und in Ordnung" sind verschiedene Aussagen. Ein Read Model, das beides zu `ok` verschmilzt, behauptet eine Prüfung, die nie stattgefunden hat.
3. **`[]` ist nicht `null`.** `[]` heißt **verifiziert leer**, `null` heißt **nicht verbürgt**. Die beiden werden nirgends ineinander normalisiert. Begründung: die übliche Vereinfachung „leer oder unbekannt ist beides nichts" würde eine degradierte Notiz als anhanglos darstellen — genau die Verwechslung, gegen die das Gate gebaut ist.
4. **Ein einziger fail-closed Mapper für jeden Notiz-Lesevorgang.** Angereichert werden `getList` und `getOne`; `getMany` und `getManyReference` bewusst nicht, weil sie keine Anzeigefläche für Anhänge sind. Weil trotzdem **alle** Lesevorgänge durch denselben Mapper laufen, landen die nicht angereicherten bei `unverified` statt unbemerkt bei `ok`. Begründung: ein Gate mit einem Umweg ist kein Gate. Wer künftig über `getMany`/`getManyReference` Anhänge anzeigen will, stellt die relationale Evidenz bewusst her — Stillschweigen wird nie als Bestätigung gelesen.
5. **Degradierte Anhänge sind lesbar, aber nicht änderbar.** Ein anhangändernder Schreibvorgang an einer Notiz, deren Zustand nicht `ok` ist, wird mit `NORA_ATTACHMENT_STATE_UNVERIFIED` abgewiesen — **vor** jeder Storage-Arbeit, damit ein abgewiesener Schreibvorgang keine Datei im Bucket zurücklässt. Nicht-anhangbezogene Änderungen (Text, Status, Datum, Kontakt-/Vorgangsbezug) bleiben möglich. Begründung: wer nicht weiß, was da liegt, darf es nicht überschreiben; die Notiz deswegen ganz zu sperren wäre Nutzerschaden ohne Sicherheitsgewinn. Das Lesen selbst ist ein passiver Zustand und wirft nichts.
6. **Eine neue Notiz wird nicht bewacht, aber auch nicht geglaubt.** Ein Neuanlegen hat keinen Vorzustand, den man prüfen könnte; das Ergebnis wird deshalb relational nachgelesen und trägt danach einen echten Zustand. Ebenso gilt: eine bereits **committete** Mutation wird nie nachträglich zum Fehler — scheitert die Verifikation, degradiert der Datensatz ehrlich nach `unverified`, statt die rohe Mutationsantwort als verifiziert auszugeben.
7. **Eine Änderung ohne Anhangbezug kostet keinen zusätzlichen Lesevorgang — außer der Vorzustand trägt nicht.** Sie übernimmt den vorhandenen kanonischen Zustand. Fehlt er, oder hat sich das Legacy-Array unter dem Schreibvorgang bewegt, wird verifiziert. Begründung: ohne diese Regel würde eine harmlose Textänderung an einer degradierten Notiz deren Legacy-Array unter dem verifizierten Feld wieder auferstehen lassen.
8. **Verifizierte Darstellung und Wiederherstellungsdarstellung sind zwei Pfade, nicht zwei Zustände eines Pfades.** Der normale Renderer nimmt ausschließlich verifizierte Anhang-Arrays entgegen; er kann Vertrauen nicht selbst entscheiden. Die Wiederherstellungsdarstellung ist schreibgeschützt: keine Bildvorschau, kein Eingabefeld, kein Entfernen, kein Weg zurück ins Formular, keine Schreibquelle. Begründung: eine nicht verbürgte Objektreferenz darf nicht als vertrauenswürdiger Inhalt eingebettet werden, und **Anzeigen ist keine Verifikation** — gerendert zu werden macht `drift` nie zu `ok`. Automatisch repariert wird nichts.
9. **Read-Model-Metadaten sind ein Lesevertrag, keine gespeicherten Felder.** Sie werden vor jedem Schreibvorgang entfernt. Die Demo-Parität (FakeRest) spricht denselben Lesevertrag, simuliert aber **keine** Projektion und keine relationale Prüfung ([`05`](05-demo-data-guidelines.md)) — sie beweist UI-Verträge, nie Production-Konsistenz.
10. **Grenze von S5.** Keine Migration, kein Schema, keine Rechte, keine RLS-, Grant- oder Storage-Policy-Änderung, kein privater Bucket (W8-E), kein Worker und keine physische Löschung (S2B), kein Rückbau des Legacy-Schreibpfads (S6). **S5 ist ausdrücklich nicht der fertige Anhang-Viewer:** die heutige Bild-/Dokumentdarstellung ist funktional, und eine spätere Produkt-/UX-Welle darf sie vollständig neu entwerfen, solange sie denselben Vertrag aus verifizierten Daten und Wiederherstellungsdaten konsumiert.
...
10. **S5 ist damit an seiner Datenvoraussetzung nicht mehr blockiert — und nicht begonnen.** „Datentor offen" ist kein Freigabe-Signal: S5 braucht eigenen Entwurf, eigene Review und eigenen Release. *(S5 ist diesen Weg gegangen und seit dem 2026-09-21 `PRODUCTION VERIFIED` — siehe Eintrag „2026-09-21 – W8-C S5" oben. Die Grenze von S4 selbst bleibt unverändert: S4 hat keine Leseseite gebracht.)*
