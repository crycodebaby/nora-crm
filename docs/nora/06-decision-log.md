...
**Nachtrag (2026-08-31): Hierarchie- und Ruhe-Iteration — LOCAL VERIFIED, UX-Abnahme offen.** Nach Product-Owner-Freigabe der empfohlenen Iteration wird die oben dokumentierte Vier-Karten-/Zweispalten-Komposition lokal durch eine gemeinsame Erfassungsfläche ersetzt. Die Reihenfolge ist jetzt eindeutig **Person → Kunde und Rolle → Erreichbarkeit → Interne Angaben**; nur eng zusammengehörige Felder stehen ab `sm` nebeneinander. Pflicht und Option werden textlich statt über starke Farbe unterschieden, Icons bleiben neutral und Nora-Rot ist auf Fokus, Auswahl und den Speicher-CTA begrenzt. Die Inhaltsbreite sinkt auf `max-w-4xl`, Abschnittstitel bilden mit 17 px/semibold die stärkste Formularebene, Haarlinien ersetzen konkurrierende Kartenschatten und der Speicher-CTA bleibt auch im mobilen Sheet mindestens 44 px hoch. Edit-/Show-Flows bleiben unverändert. Scope weiterhin ausschließlich UI/i18n: keine Persistenz-, Datenmodell-, Routing-, RBAC- oder Backendänderung. Verifiziert mit ESLint auf den geänderten TS-/TSX-Dateien, `npm run typecheck`, `npm run build` sowie vollständiger Vitest-Suite (85 Dateien, 693 bestanden, 1 bewusst übersprungen). Die gestylte Live-/Browser-Abnahme war in dieser Session durch die lokale Browser-URL-Richtlinie nicht möglich; deshalb bewusst **nicht deployt** und noch nicht als UX-abgenommen markiert.
