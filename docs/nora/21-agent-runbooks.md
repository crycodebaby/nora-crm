...
| `SECURITY DEFINER`, `security_invoker`, Grants, RLS, neue Tabelle/View/Function in `public`, Storage-Policies, Bucket `attachments`, Tabelle `public.attachments` und die Löschintent-Warteschlange | [4. Security und Zugriff](#4-security-und-zugriff) |
...
- [ ] **Die Warteschlange hat seit S2A1 einen Erzeuger, aber keinen Konsumenten.** Ein Eintrag ist ein Lösch**vorhaben**, keine Erlaubnis: **nie** einen Worker, eine Claim-/Ack-/Fail-RPC oder einen Storage-`DELETE` ergänzen, ohne vorher den Liveness-Resolver (S2A2) **und** die atomare Referenzschreibung (S3) zu haben — sonst löscht Nora Dateien, die wieder referenziert wurden ([`17`](17-known-issues-and-planned-waves.md) H.1, Re-Referenzierungs-Rennen). In die Capture-Funktion kommt **nie** ein HTTP-, `pg_net`-, Storage- oder Edge-Aufruf zurück
