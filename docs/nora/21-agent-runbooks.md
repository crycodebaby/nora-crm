...
Stand: 2026-09-19 · Load-Klasse: **CONDITIONAL** — dieses Dokument wird **nie vollständig** als Standardkontext geladen.
...
| `SECURITY DEFINER`, `security_invoker`, Grants, RLS, neue Tabelle/View/Function in `public`, Storage-Policies, Bucket `attachments`, Tabelle `public.attachments`, die Löschintent-Warteschlange, der Anhang-Liveness-Resolver und die Serialisierung je Objektschlüssel | [4. Security und Zugriff](#4-security-und-zugriff) |
...
- [ ] **`public.attachments` ist leer und unverdrahtet** (W8-C S1), seit S3A für API-Rollen schreibgeschützt. Wer sie anbindet oder befüllt, baut S3B ff. — eine eigene Welle mit eigener Abnahme, keine Nebenwirkung. Bis dahin **nie** behaupten, Nora nutze die Tabelle, und ein Zeilen-`DELETE`/`CASCADE` **nie** als „die Datei wird gelöscht" beschreiben. Den entzogenen direkten `INSERT`/`DELETE`-Grant für `authenticated` **nicht** „wiederherstellen": der künftige Schreiber ist die Datenbank selbst ([`22`](22-security-and-access.md) Abschnitt 6.6)
