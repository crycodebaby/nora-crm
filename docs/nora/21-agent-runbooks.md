...
- [ ] **Die Warteschlange hat seit S2A1 einen Erzeuger, aber keinen Konsumenten; der Resolver (S2A2.1) hat keinen produktiven Aufrufer.** Ein Eintrag ist ein Lösch**vorhaben**, ein `dead` eine Momentbeobachtung — beides keine Erlaubnis: **nie** einen Worker, eine Claim-/Lease-/Ack-/Fail-RPC oder einen Storage-`DELETE` ergänzen, ohne den Ausführungsvertrag (S2A2.2) **und** die atomare Referenzschreibung (S3) zu haben — sonst löscht Nora Dateien, die wieder referenziert wurden ([`17`](17-known-issues-and-planned-waves.md) H.1, Re-Referenzierungs-Rennen). `skipped_live` nicht als „gelöscht" oder „dauerhaft sicher" lesen. In die Capture-Funktion und den Resolver kommt **nie** ein HTTP-, `pg_net`-, Storage- oder Edge-Aufruf; ein `exception when others`, das Fehler in `unknown` oder `dead` verwandelt, ist ein Defekt
