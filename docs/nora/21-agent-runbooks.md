...
Stand: 2026-09-18 · Load-Klasse: **CONDITIONAL** — dieses Dokument wird **nie vollständig** als Standardkontext geladen.
...
| `SECURITY DEFINER`, `security_invoker`, Grants, RLS, neue Tabelle/View/Function in `public`, Storage-Policies, Bucket `attachments`, Tabelle `public.attachments`, die Löschintent-Warteschlange und der Anhang-Liveness-Resolver | [4. Security und Zugriff](#4-security-und-zugriff) |
...
- [ ] **`public.attachments` ist leer und unverdrahtet** (W8-C S1). Wer sie anbindet oder befüllt, baut S3 ff. — eine eigene Welle mit eigener Abnahme, keine Nebenwirkung. Bis dahin **nie** behaupten, Nora nutze die Tabelle, und ein Zeilen-`DELETE`/`CASCADE` **nie** als „die Datei wird gelöscht" beschreiben
