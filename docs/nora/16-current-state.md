...
17. **Kontakterstellung UI-Polish** (2026-08-28): mobile-first Formularhierarchie („Person“, „Kundenbezug“, „Kontaktmöglichkeiten“, eingeklappte „Weitere Angaben“), feste Nora-Primäraktion, mobile Kundenwahl als Bottom Sheet mit getrennter Neuanlage sowie verdichtete iPad-Kopfzeile. Reine UI-/i18n-Änderung, keine Persistenz- oder Routingänderung. **LOKAL IMPLEMENTIERT UND VERIFIZIERT, NOCH NICHT ALS PRODUCTION-DEPLOYMENT BEHAUPTET.** Siehe Decision Log „2026-08-28 – Kontakterstellung UI-Polish“.
...
5. ~~Idempotency für retry-fähige/externe Write Commands (`CreateQuickCaptureCase`, `CreateCustomerFromContact`)~~ — **Idempotency Wave, 2026-08-29, implementiert und lokal verifiziert** (Migration `20260829120000_nora_idempotency_core.sql`, `NORA_IDEMPOTENCY_CONFLICT`, siehe Decision Log). Noch NICHT auf Produktion angewendet/deployed — Migration liegt bereit, kein Push in dieser Session.
