...
- Clients (`authenticated`) können **nicht** direkt INSERT/UPDATE/DELETE; seit Security Hardening Wave 0 (2026-09-04) besitzt `authenticated` auf `audit_events` **genau `SELECT`** (auch `TRUNCATE`/`TRIGGER`/`REFERENCES`/`MAINTAIN` entzogen — `TRUNCATE` umgeht RLS und Row-Trigger).
- **Append-only gilt seit Security Hardening Wave 1 (`PRODUCTION VERIFIED` 2026-09-07) auch im Objekt-ACL:** `service_role` hat auf `audit_events` `SELECT` und `INSERT` — **kein** `UPDATE`, **kein** `DELETE`, **kein** `TRUNCATE`. Das frühere akzeptierte Restrisiko „`service_role` behält `TRUNCATE`" ist damit aufgelöst; die Privilegien sagen jetzt dasselbe wie die `prevent_audit_events_update`/`_delete`-Guards. Unberührt bleibt die letzte Instanz: der Tabellen-Owner `postgres` (und damit eine Migration) kann weiterhin alles — Anwendungs-Unveränderlichkeit ist nicht Owner-Unveränderlichkeit.
