...
| P13 | PWA: manueller **Update-Einstieg** (Entry Point) | Ein persistentes Nora-Bedienelement öffnet die bestehende Update-Fläche erneut, nachdem der Nutzer „Später" gewählt hat. Heute erscheint die Fläche nur automatisch nach `DISMISS_RESHOW_AFTER_MS` (2 h) wieder; kein Bedienelement | **PLANNED** — gezielte UX-Welle. Das vorhandene Aktualisieren-Bedienelement oben rechts (`RefreshButton` in `Header.tsx`, heute Daten-Refresh) wird als Wiedereinstieg evaluiert. Nicht implementiert; nutzt `usePwaUpdate()`, kein zweiter Zustand |
| P13a | PWA: manueller **Update-Check** | Die App fragt die Service-Worker-Registrierung ausdrücklich nach einer neuen Version (`registration.update()`), statt nur auf Tab-Rückkehr und stündlich zu prüfen | **OPTIONAL / PLANNED** — nur, wenn er den bestehenden PWA-State-Contract wiederverwendet; kein zweiter Zustandsautomat, kein zweites Polling. Nicht dasselbe wie P13 |
