...
### H.1 Anhänge / Storage — offen nach W8-B, W8-C S1, S2A1, S2A2.1, S2A2.2 und S3A (2026-09-19)
...
- **Verwaiste Storage-Objekte** — `ACCEPTED LIMITATION`, unverändert nach S1, S2A1, S2A2.1, S2A2.2 und S3A. Es gibt weiterhin **keinen** physischen Löschpfad: das Löschen einer Notiz entfernt die Datei nicht, und das gilt ausdrücklich auch für die FK-`CASCADE`s auf `public.attachments` — sie löschen die **Metadatenzeile**, nicht das Objekt im Bucket. S2A1 hält das Vorhaben fest, S2A2.1 beobachtet, S2A2.2 verwaltet Aufträge, S3A serialisiert — keiner löscht. Unverändert gilt auch: ein Upload landet **vor** dem Datenbank-Schreibvorgang im Bucket; scheitert der Schreibvorgang danach, bleibt das Objekt verwaist. S3A löst das nicht (`DEFERRED`). Der Liveness-Zensus vom 2026-09-18 zählt 43 Objekte, davon **8 aktuell unreferenzierte Verwaisungskandidaten** (`dead`); sie sind **nicht** bereinigt, **nicht** als entbehrlich bestätigt, und es gibt keine Fähigkeit, sie zu entfernen. Der alte Trigger-/`pg_net`-/Edge-Pfad wird nicht reaktiviert.
- **Der Löschpfad ist in Schnitte zerlegt — sie werden nicht zusammengezogen.** Reihenfolge und Zustand:
  ...
  - **S3B Projektion Notiz-JSON → `public.attachments`** — `OPEN / NEXT`, nicht begonnen. Die Datenbank soll die Anhangverweise der Notiz-Arrays selbst in `public.attachments` abbilden; S3A hat dafür den direkten API-Schreibpfad bereits entzogen. Ableitungsregeln, Validierung der Legacy-Formen und die Oberflächenabbildung des Zulassungsfehlers `NORA_ATTACHMENT_STORAGE_KEY_PENDING_DELETION` sind **nicht entworfen**. Bis S3B gibt es keinen Trigger auf `contact_notes`/`deal_notes` und kein Dual-Write.
  - **S4 Backfill der Bestandsanhänge + Konsistenzprüfung** — `FUTURE`, nicht begonnen.
  - **S5 Umschalten der Leseseite** — `FUTURE`, nicht begonnen.
  - **S6 Abschaltung des Legacy-JSON-Schreibpfads** — `FUTURE`, nicht begonnen.
  - **S2B physischer Löschworker (Storage-`DELETE`)** — `GATED / FUTURE`. S2B bringt den echten Worker und erst mit ihm eine eng geschnittene `service_role`-Ausführungsgrenze, einen Pfad zu `done` sowie den Vertrag für externe Seiteneffekte. Gates (alle unten): Seiteneffekt-Abgrenzung, getrennte Transaktionen für Claim und Inspektion, `done`-Index, quellenübergreifende Schlüssel. Die bloße Existenz der Warteschlange, ihres Ausführungsvertrags oder der S3A-Serialisierung ist **kein** Grund, einen Worker scharf zu schalten.
...
- **Quellenübergreifend geteilte Schlüssel** — `OPEN (LOW)`, **Carry-forward S2B/S4**. Schreibvorgänge auf die übrigen registrierten Referenzflächen (Legacy-JSON-Arrays bis S3B, Kundenlogos, Branding, Avatare) nehmen die Schlüsselsperre nicht. Im normalen Betrieb entstehen dort frische UUID-Schlüssel; ein Schlüssel, der zugleich in `public.attachments` und in einer anderen Quelle steht, entsteht nur durch gezielt gebaute Schreibvorgänge. Vor physischer Löschung muss S2B bzw. der Backfill (S4) diesen Fall abdecken oder ausschließen.
...
- **Das Attachment-Modell ist ein leeres Fundament, keine Fläche** — `PLANNED DOMAIN WAVE` (W8-C S3B ff.), S1, S2A1, S2A2.1, S2A2.2 und S3A erledigt. Seit S1 existiert die Entität `public.attachments` mit Attachment-Id, Ownership auf Datensatzebene (Notiz-XOR) und provider-neutraler `storage_key`-Identität; seit S3A ist sie für API-Rollen schreibgeschützt und durch Zulassung und Unveränderlichkeit abgesichert. **Noch nicht vorhanden und nicht begonnen:** Projektion aus den Notiz-JSON-Arrays (S3B), Migration der Bestandsanhänge (S4), Umschalten der Leseseite (S5), Abschaltung des Legacy-Schreibpfads (S6), physische Storage-Bereinigung (S2B), atomare Notiz-RPC sowie eine Audit-Spur für das Hinzufügen oder Entfernen eines Anhangs. Bis dahin bleiben die JSON-Arrays am Notizdatensatz die live genutzte Darstellung, der vom Client gelieferte `src`/`path` ein Legacy-Feld und Provider-URLs persistiert.
