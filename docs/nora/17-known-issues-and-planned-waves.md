...
### H.1 Anhänge / Storage — offen nach W8-C S6-A (Production Apply 2026-09-25)
...
Ebenfalls **nicht** offen ist **W8-C S3B** (Note Attachment Projection, `CLOSED / PRODUCTION VERIFIED / LEDGER REPO-ALIGNED` 2026-09-19, Ledger 66/66 deckungsgleich): Datenbank-Trigger projizieren das Anhang-Array einer Notiz bei `INSERT` und bei jeder Array-Änderung in `public.attachments` — in derselben Transaktion, als minimale Differenz gegen die tatsächlichen Zeilen, unter einer zentralen Grammatik (v1), ohne Dual-Write und ohne direkte Schreibrechte irgendeiner API-Rolle. Die beiden über Notiz-Schreibvorgänge erreichbaren Fehlercodes sind auf die Oberfläche abgebildet. Contract [`22`](22-security-and-access.md) Abschnitt 6.11, Invarianten [`03`](03-data-model-guardrails.md) §1.8, Entscheidung [`06`](06-decision-log.md) „2026-09-19 – W8-C S3B", Evidenz `releases/2026-09.md`. **Projektion ist kein Backfill** — S3B selbst hat keine Bestandsnotiz zurückprojiziert (das hat S4 getan, unten), und gelöscht wird weiterhin nichts.

Ebenfalls **nicht** offen ist **W8-C S4** (Attachment Consistency Backfill, `PRODUCTION VERIFIED` 2026-09-20): die von S3B unberührten Bestandsnotizen sind in `public.attachments` nachprojiziert, Notiz für Notiz über den vorhandenen Kern `nora_private.reconcile_note_attachments(...)` unter einer Sperre der Notizzeile. Bei der Production-Verifikation war der kanonische Konsistenzprüfer `GREEN` (alle zwölf blockierenden Klassen 0, `BACKFILL_CANDIDATE_REMAINING` = 0). S4 brachte **keine** Migration, keine Zeile in `src/**`, keine Grant- oder Policy-Änderung und keine Anwendungsfläche — ausgeliefert wurde Operator-Werkzeug unter `supabase/maintenance/` samt Verifikationssuiten. Contract [`22`](22-security-and-access.md) Abschnitt 6.11, Invarianten [`03`](03-data-model-guardrails.md) §1.8, Entscheidung [`06`](06-decision-log.md) „2026-09-20 – W8-C S4", Evidenz `releases/2026-09.md`. **Backfill ist keine Leseseite** — die hat S5 gebracht (unten); das Legacy-JSON ist nicht abgelöst (S6), und gelöscht wird nach wie vor nichts (S2B).

Ebenfalls **nicht** offen ist **W8-C S5** (Relational Attachment Read Gate, `CLOSED / PRODUCTION VERIFIED` 2026-09-21, Laufzeit `3f9dd9ed`, **keine Migration**, Ledger unverändert 66): die Anwendung liest Notiz-Anhänge nur noch als **verifiziertes Read Model**. `public.attachments` ist die **Vertrauensquelle** — verbürgt sind Mitgliedschaft, `storage_key`, Notiz-Besitz, Titel und Typ; Array-Reihenfolge, `src`, `rawFile` und geduldete Legacy-Felder bleiben bis S6 beim Notiz-JSON. Drei fail-closed Zustände (`ok` / `drift` / `unverified`), `[]` (verifiziert leer) ist nie `null` (nicht verbürgt), `getList`/`getOne` sind angereichert und `getMany`/`getManyReference` bewusst nicht, eine anhangändernde Änderung an einer nicht verbürgten Notiz wird vor jeder Storage-Arbeit abgewiesen, und degradierte Anhänge erscheinen in einer schreibgeschützten Wiederherstellungsdarstellung. Contract [`22`](22-security-and-access.md) Abschnitt 6.12, Invarianten [`03`](03-data-model-guardrails.md) §1.8, Entscheidung [`06`](06-decision-log.md) „2026-09-21 – W8-C S5", Evidenz `releases/2026-09.md`. **Ein Lese-Gate ist keine Ablösung** — der Legacy-JSON-Schreibpfad besteht unverändert (S6-B1/B2), die Zeilen sind **nicht** die vollständige Anhang-Nutzlast, gelöscht wird weiterhin nichts (S2B), der Bucket bleibt öffentlich (W8-E), und **die Anhang-Darstellung ist nicht abgenommen** (eigener Punkt unten). Offen bleiben:

Ebenfalls **nicht** offen ist **W8-C S6-A** (Attachment Ordinal, Production Apply 2026-09-25, danach unabhängig `CLOSED / PRODUCTION VERIFIED`, Migration `20260922140000_nora_attachment_ordinal`, Ledger 68/68): `public.attachments` trägt jetzt `ordinal integer NOT NULL` ohne Default und eine nicht aufschiebbare Eindeutigkeitsregel pro Notiz (`UNIQUE NULLS NOT DISTINCT`). Die 31 Bestandszeilen wurden in Legacy-Reihenfolge befüllt; der kanonische Prüfer war `GREEN`, die Warteschlange leer. Der Commit `eba95108` ändert keine Zeile in `src/**`; Lesepfad, JSON-Schreibpfad, Bucket und physischer Löschpfad bleiben unverändert. S6-A liefert die relationale Ordnungsgrundlage, **nicht** den Autoritätswechsel. Evidenz: `releases/2026-09.md`.

- **Der Bucket `attachments` ist weiterhin öffentlich (`public = true`)** — `OPEN`, bewusst getragen. Wer einen Objektschlüssel kennt, lädt die Datei ohne Anmeldung herunter; das gilt auch für Bestandsobjekte und für ausgeschiedene Mitarbeiter, die einen Schlüssel besitzen. W8-B hat die Storage-**API**-Autorisierung gehärtet, nicht die Vertraulichkeit vorhandener Objekt-URLs. Die Umstellung auf einen privaten Bucket (samt signierten URLs, Migration der Bestandsschlüssel und Logo-Sonderfall) ist eine **eigene Welle (W8-E)**, nicht designt. **S5 hat daran nichts geändert:** ein verifizierter Anhang wird weiterhin über dieselbe öffentliche Objekt-URL angezeigt wie vorher — das Lese-Gate beweist Zugehörigkeit, nicht Vertraulichkeit.
- **Verwaiste Storage-Objekte** — `ACCEPTED LIMITATION`, unverändert nach S1, S2A1, S2A2.1, S2A2.2, S3A, S3B und S4. Es gibt weiterhin **keinen** physischen Löschpfad: das Löschen einer Notiz entfernt die Datei nicht, und das gilt ausdrücklich auch für die FK-`CASCADE`s auf `public.attachments` — sie löschen die **Metadatenzeile**, nicht das Objekt im Bucket. S2A1 hält das Vorhaben fest, S2A2.1 beobachtet, S2A2.2 verwaltet Aufträge, S3A serialisiert, S3B projiziert, S4 hat den Bestand nachprojiziert — keiner löscht. Unverändert gilt auch: ein Upload landet **vor** dem Datenbank-Schreibvorgang im Bucket; scheitert der Schreibvorgang danach, bleibt das Objekt verwaist. Weder S3A noch S3B lösen das (`DEFERRED`); seit S3B kann auch eine abgewiesene Projektion einen solchen Schreibvorgang scheitern lassen. Der Liveness-Zensus vom 2026-09-18 zählt 43 Objekte, davon **8 aktuell unreferenzierte Verwaisungskandidaten** (`dead`); sie sind **nicht** bereinigt, **nicht** als entbehrlich bestätigt, und es gibt keine Fähigkeit, sie zu entfernen. Der alte Trigger-/`pg_net`-/Edge-Pfad wird nicht reaktiviert.
- **Der Löschpfad ist in Schnitte zerlegt — sie werden nicht zusammengezogen.** Reihenfolge und Zustand:
  ...
  - **S4 Backfill der Bestandsanhänge + globale Konsistenzprüfung** — `CLOSED / PRODUCTION VERIFIED` (2026-09-20), siehe oben. Die von S3B unberührten Bestandsnotizen sind projiziert; der kanonische Konsistenzprüfer war danach `GREEN`. Ohne Migration, ohne `src/**`, ohne Rechteänderung. **Datenoperation, kein Contract** — die erreichte Parität ist eine verifizierte Momentaufnahme, kein Constraint.
  - **S5 Umschalten der Leseseite** — `CLOSED / PRODUCTION VERIFIED` (2026-09-21), siehe oben. Die Anwendung liest `public.attachments` als Vertrauensquelle eines verifizierten Read Models; anhangändernde Schreibvorgänge an nicht verbürgten Notizen werden abgewiesen. **Gate, nicht Ablösung** — S5 hat keine Migration, keine Rechteänderung und keinen Schreibpfad gebracht.
  - **S6-A relationale Ordnungsgrundlage** — `CLOSED / PRODUCTION VERIFIED` (Production Apply 2026-09-25), siehe oben.
  - **S6-B1 Wechsel der relationalen Leseautorität** — `FUTURE`, nicht implementiert. Der S5-Lesepfad nutzt `ordinal` noch nicht als Autorität.
  - **S6-B2 sicherer Wechsel der Schreib- und Ordnungsautorität** — `FUTURE`, nicht implementiert. Das Notiz-JSON bleibt Schreibpfad und Laufzeitautorität für Reihenfolge, `src`, `rawFile` und geduldete Legacy-Felder. Beide Schnitte brauchen eigene Entscheidung, Review und Release; S6-A ist keine Implementierungsfreigabe.
  - **S2B physischer Löschworker (Storage-`DELETE`)** — `GATED / FUTURE`. Physische Storage-Löschung ist **nicht** aktiv. S2B bringt den echten Worker und erst mit ihm eine eng geschnittene `service_role`-Ausführungsgrenze, einen Pfad zu `done` sowie den Vertrag für externe Seiteneffekte. Gates (alle unten): Seiteneffekt-Abgrenzung, getrennte Transaktionen für Claim und Inspektion, veraltetes Formular (Lost Update), `done`-Index, quellenübergreifende Schlüssel. Die bloße Existenz der Warteschlange, ihres Ausführungsvertrags, der S3A-Serialisierung, erster echter Vorhaben seit S3B oder des S5-Lese-Gates ist **kein** Grund, einen Worker scharf zu schalten. **S5 hat kein Gate geschlossen** — es löscht nichts, beansprucht nichts und ändert an der Warteschlange nichts.
- **Fenster S3B → S4: unberührte Bestandsnotizen waren unprojiziert** — `CLOSED` (2026-09-20), **historisch**. Zwischen dem S3B-Apply (2026-09-19) und dem S4-Backfill (2026-09-20) wurde eine Bestandsnotiz erst beim ersten Anhang-ändernden Schreibvorgang projiziert, dann aber vollständig (First-Touch). Bekannte Lücke **dieses Fensters**, als Bedingung formuliert: **wäre** ein Anhang aus einer Bestandsnotiz entfernt worden, **bevor** sie je projiziert war, hätte es keine Zeile gegeben, die gelöscht würde — die Erfassung (S2A1) hätte für diese historische Referenz kein Löschvorhaben festhalten können. Das wäre keine Verschlechterung gegenüber dem Stand vor S3B gewesen (dort entstand nie ein Vorhaben) und hätte höchstens ein weiteres Objekt ohne Vorhaben zur Folge gehabt, nie eine falsche Löschung. **Beobachtet wurde ein solcher Fall nicht:** zwischen dem Bestandszensus vor dem S3B-Apply (2026-09-19) und dem S4-Preflight (2026-09-20) blieben Anzahl der Anhangelemente (31) und Anzahl verschiedener Schlüssel (31) unverändert — im verifizierten Production-Bestand gibt es keinen Hinweis auf einen Referenzverlust in diesem Fenster. Der Eintrag bleibt als Einordnung stehen; mit dem Liveness-Zensus vom 2026-09-18 (oben) hat er nichts zu tun — jene Beobachtung liegt **vor** dem Fenster. First-Touch selbst bleibt der Mechanismus für jede künftige unprojizierte Notiz ([`22`](22-security-and-access.md) Abschnitt 6.11).
- **S2B-Designgate: veraltetes Formular überschreibt eine neuere Anhangliste** — `OPEN`, **S2B-Gate**, in der unabhängigen Review von S3B festgehalten. Notiz-Schreibvorgänge haben keine optimistische Nebenläufigkeitskontrolle; der Lost Update selbst ist vorbestehend. Neu seit S3B: die Projektion bildet den Überschreibvorgang getreu ab und erfasst für die verdrängten Anhänge ein Löschvorhaben. Solange nichts physisch löscht, ist das rückholbar. Bevor S2B einen Storage-`DELETE` aktiviert, muss dieses Szenario bewusst gelöst oder mit einem sicheren Entwurf getragen werden. Nicht entworfen; S4 hat es **nicht** angefasst und war nie dafür vorgesehen.
...
- **Quellenübergreifend geteilte Schlüssel** — `OPEN (LOW)`, **Carry-forward S2B**. Verweise außerhalb von `public.attachments` nehmen die Schlüsselsperre nicht: Kundenlogos, Branding, Avatare. Notiz-Anhänge nehmen seit S3B über die Projektion teil, und seit S4 gilt das auch für den Bestand. Im normalen Betrieb entstehen dort frische UUID-Schlüssel; ein Schlüssel, der zugleich in `public.attachments` und in einer anderen Quelle steht, entsteht nur durch gezielt gebaute Schreibvorgänge. Der S4-Preflight hat diesen Fall für den Bestand **geprüft und ausgeschlossen** (`CROSS_SOURCE_KEY` = 0 bei der Verifikation am 2026-09-20) — das ist eine datierte Beobachtung, kein Schutzmechanismus. Vor physischer Löschung muss S2B den Fall weiterhin abdecken oder ausschließen.
...
- **Das Attachment-Modell hat seit S6-A eine relationale Ordnungsgrundlage, aber noch keine abgeschlossene Domäne** — `PLANNED DOMAIN WAVE`. `public.attachments` enthält Mitgliedschaft, Notiz-Besitz, `storage_key`, Metadaten und `ordinal`; die Datenbank projiziert die Zeilen, die Anwendung nutzt sie seit S5 als Vertrauensquelle des Lesepfads. **S6-B1 und S6-B2 sind nicht implementiert:** Legacy-JSON bleibt der aktuelle Read-/Write-Übergang, insbesondere für angezeigte Reihenfolge, `src`, `rawFile` und geduldete Felder. Getrennt offen sind physische Storage-Bereinigung (S2B), privater Bucket (W8-E), Anhang-Viewer (eigener Produkt-/Redesign-Stream), KI-/Inhaltsmetadaten, atomare Notiz-RPC und Anhang-Audit. Keine dieser offenen Wellen ist durch S6-A freigegeben.
- **Anhang-Darstellung / Viewer-UX** — `PLANNED FOLLOW-UP` (Produkt/UX), nicht designt, **keine Wellennummer vergeben**. S5 hat die Vertrauensgrenzen und das sichere Rendern etabliert, aber **nicht** versucht, die Darstellung selbst abzuschließen. Offen und bewusst nicht entschieden: Bilddarstellung und Galerie, Vorschau/Lightbox, Dokumentzeilen bzw. -karten, Dateinamen- und Metadatendichte, Icons, Öffnen-/Herunterladen-Aktionen, mobile Darstellung, Mehrdatei-Layout und Viewer-Interaktion. **Die heutige Bild-/Link-Darstellung ist funktional, kein Design-System-Invariant** ([`02`](02-design-system.md) kanonisiert sie nicht). Eine spätere UX-Welle darf diese Flächen vollständig neu entwerfen, **ohne** den S5-Vertrauensvertrag zu öffnen — sie konsumiert weiterhin dieselbe Trennung aus verifizierten Daten und schreibgeschützten Wiederherstellungsdaten ([`22`](22-security-and-access.md) Abschnitt 6.12). Das ist **nicht** S6, **nicht** S2B und **nicht** W8-E.
- **Keine Regressionstests für drei fail-closed Randfälle des Lese-Gates** — `INFO / TEST-DEBT`, nicht blockierend, festgestellt bei S5 (2026-09-21). Ohne eigenen Test sind: doppelter Anhangpfad im Legacy-Array, doppelter `storage_key` in den relationalen Zeilen und eine relationale Zeile mit fehlendem bzw. leerem `storage_key`. Das Implementierungsverhalten ist in allen drei Fällen korrekt und fail-closed (`drift`) — es fehlt nur die ausdrückliche Absicherung gegen eine spätere Regression. **Abhilfe** (kleiner Test-Slice, nicht beauftragt): die drei Fälle in die vorhandene Read-Model-Suite aufnehmen.
- **Ein Nullsubmit an einer bereits degradierten Notiz kann einen überflüssigen Verifikations-Lesevorgang auslösen** — `INFO / PERFORMANCE (LOW)`, festgestellt bei S5. Kein Korrektheits- und kein Sicherheitsproblem: der Zustand bleibt in jedem Fall richtig, es entsteht höchstens ein zusätzlicher `GET`. Keine Abhilfe beauftragt.
- **`AttachmentNote` und der `rawFile`-Typ von react-admin passen nicht exakt zusammen** — `INFO / TYPE-DEBT`, **vorbestehend**, nicht durch S5 entstanden und von S5 bewusst nicht mitgefixt.
...
- **Demo/FakeRest bildet die Projektion nicht nach** — `INFO / DEBT`, unverändert nach S5. FakeRest kennt weder `public.attachments` noch Grammatik, Zulassung oder relationale Paritätsprüfung; Demo-Anhänge tragen keinen `path`. Seit S5 spricht die Demo **den Lesevertrag** mit (jede Demo-Notiz trägt den Zustand `ok`, Teilaktualisierungen erfinden keine Anhänge, und Read-Model-Metadaten werden nicht als Demo-Geschäftsdaten persistiert) — **sie beweist damit Anwendungs-/UI-Verträge, nie relationale Production-Konsistenz.** Ein `ok` aus der Demo ist kein geprüfter Zustand, sondern eine Setzung. Das ist nach [`05`](05-demo-data-guidelines.md) und [`03`](03-data-model-guardrails.md) §5 zulässig (Parität, kein Ersatz) und kein Beleg für eine Production-Lücke. Eine echte Projektionsparität in FakeRest wäre eine eigene Welle.
...
### I.8 Relative Asset-Auflösung bei direktem Aufruf eines Nicht-Hash-Tiefpfads

**Status: `OPEN (LOW)` — vorbestehende Build-/Routing-Schuld**, beobachtet während des W8-C-S5-Production-Smokes
(2026-09-21); **kein** S5-Befund und keine S5-Regression.

Der Vite-Build läuft mit `base: "./"`. Wird eine tiefe Nicht-Hash-URL **direkt** im Browser geöffnet, löst das
Dokument seine relativen Asset-Pfade gegen diesen Tiefpfad auf statt gegen den Anwendungsstamm; die Anwendung
startet dann nicht. Das normale Nora-Routing ist davon **nicht** betroffen: es läuft über den Hash-Router mit
deutschen Aliassen ([`04`](04-routing-i18n.md)), und dort steht der Tiefpfad hinter dem `#`.

**Abgrenzung — kein Beleg für eine unterstützte Route.** Dass ein solcher Pfad überhaupt aufgerufen werden kann,
macht ihn nicht zu einer Nora-Route; englische Tiefpfade wie `/contacts/…` sind **keine** unterstützten Einstiege.
