...
- **Weitere Edge Functions im Repo sind nicht deployt** (`calendar-*`, `merge_contacts`, `update_password`, `postmark`, `mcp`) — Kontext, kein Bug; nur `users` (v9) und `brevo-email-events` (v2) sind live (read-only verifiziert 2026-09-10). Edge Functions werden nie von Vercel ausgeliefert. `delete_note_attachments` steht hier nicht mehr: die Function war nie deployt und ist mit W8-B aus dem Repository entfernt (H.1).
...
### H.1 Anhänge / Storage — offen nach W8-B, W8-C S1, S2A1 und S2A2.1 (2026-09-18)
