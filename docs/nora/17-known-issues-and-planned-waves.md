...
### H.1 Anhänge / Storage — offen nach W8-B und W8-C S1 (2026-09-17)
...
- **Verwaiste Storage-Objekte** — `ACCEPTED LIMITATION`, unverändert nach S1. Es gibt weiterhin **keinen** physischen Löschpfad: das Löschen einer Notiz entfernt die Datei nicht, und das gilt ausdrücklich auch für die neuen FK-`CASCADE`s auf `public.attachments` — sie löschen die **Metadatenzeile**, nicht das Objekt im Bucket. Bestehende Verwaisungskandidaten sind **nicht** bereinigt. Der neue Löschpfad ist **W8-C S2** (Löschwarteschlange + Worker auf Basis dieser Zeilenlöschung), nicht begonnen; der alte Trigger-/`pg_net`-/Edge-Pfad wird nicht reaktiviert.
- **Das Attachment-Modell ist ein leeres Fundament, keine Fläche** — `PLANNED DOMAIN WAVE` (W8-C S2 ff.), S1 erledigt. Seit S1 existiert die Entität `public.attachments` mit Attachment-Id, Ownership auf Datensatzebene (Notiz-XOR) und provider-neutraler `storage_key`-Identität. **Noch nicht vorhanden und nicht begonnen:** Dual-Write, Migration der Bestandsanhänge, Umschalten der Leseseite, Abschaltung des Legacy-Schreibpfads, physische Storage-Bereinigung, atomare Notiz-RPC sowie eine Audit-Spur für das Hinzufügen oder Entfernen eines Anhangs. Bis dahin bleiben die JSON-Arrays am Notizdatensatz die live genutzte Darstellung, der vom Client gelieferte `src`/`path` ein Legacy-Feld und Provider-URLs persistiert.
