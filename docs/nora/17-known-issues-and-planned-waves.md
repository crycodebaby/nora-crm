...
### H.1 Anhänge / Storage — offen nach W8-B, W8-C S1 und W8-C S2A1 (2026-09-17)
...
- **Verwaiste Storage-Objekte** — `ACCEPTED LIMITATION`, unverändert nach S1 und S2A1. Es gibt weiterhin **keinen** physischen Löschpfad: das Löschen einer Notiz entfernt die Datei nicht, und das gilt ausdrücklich auch für die FK-`CASCADE`s auf `public.attachments` — sie löschen die **Metadatenzeile**, nicht das Objekt im Bucket. Bestehende Verwaisungskandidaten sind **nicht** bereinigt. S2A1 hat daran nichts geändert: es hält das Vorhaben fest, es führt es nicht aus. Der alte Trigger-/`pg_net`-/Edge-Pfad wird nicht reaktiviert.
- **Der Löschpfad ist in drei Schnitte zerlegt — sie werden nicht zusammengezogen.** Reihenfolge und Zustand:
  - **S2A1 Erfassung** — `CLOSED / PRODUCTION VERIFIED` (2026-09-17), siehe oben.
  - **S2A2 Ausführungsvertrag der Warteschlange** — `OPEN`, nächster Planungsschnitt. Erwarteter Umfang: ein **zentraler Liveness-Resolver** (ist der Schlüssel irgendwo noch referenziert? inkl. Legacy-JSON und URL-abgeleiteter Verweise), Claim-/Lease-/Ack-/Fail-Vertrag, Retry-Semantik und eine minimale `service_role`-Ausführungsgrenze. **Auch S2A2 löscht noch kein Storage-Objekt.**
