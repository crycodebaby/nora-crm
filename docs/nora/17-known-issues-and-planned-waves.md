...
### H.1 Anhänge / Storage — offen nach W8-B, W8-C S1, S2A1, S2A2.1, S2A2.2, S3A, S3B, S4 und S5 (2026-09-21)
...
- **Der Löschpfad ist in Schnitte zerlegt — sie werden nicht zusammengezogen.** Reihenfolge und Zustand:
  ...
  - **S6 Abschaltung des Legacy-JSON-Schreibpfads** — `FUTURE`, **nicht begonnen**. Der Notiz-JSON-Schreibpfad ist unverändert aktiv und bleibt die Autorität für Array-Reihenfolge, `src`, `rawFile` und geduldete Legacy-Felder. Erst S6 verschiebt diese Autorität; S5 hat sie ausdrücklich **nicht** verschoben. Eigener Entwurf, eigene Review, eigener Release.
...
- **Das Attachment-Modell hat seit S5 eine Lesefläche, aber noch keine abgeschlossene Domäne** — `PLANNED DOMAIN WAVE`; S1, S2A1, S2A2.1, S2A2.2, S3A, S3B, S4 und S5 erledigt. Vorhanden: die Entität `public.attachments` mit Attachment-Id, Ownership auf Datensatzebene (Notiz-XOR) und provider-neutraler `storage_key`-Identität, für API-Rollen schreibgeschützt, durch Zulassung und Unveränderlichkeit abgesichert, seit S3B von der Datenbank aus den Notiz-Arrays projiziert, seit S4 auch für den Bestand — und seit S5 von der Anwendung als **Vertrauensquelle** des Anhang-Lesepfads genutzt. **Noch nicht vorhanden und nicht begonnen:** Abschaltung des Legacy-Schreibpfads (S6), physische Storage-Bereinigung (S2B), privater Bucket (W8-E), ein abgenommener Nora-Anhang-Viewer (eigener Punkt unten), KI-/Inhaltsmetadaten, atomare Notiz-RPC sowie eine Audit-Spur für das Hinzufügen oder Entfernen eines Anhangs. **S5 hat den Schreibvertrag nicht angefasst:** die JSON-Arrays am Notizdatensatz bleiben Schreibpfad und Autorität für Reihenfolge, `src`, `rawFile` und geduldete Legacy-Felder, und Provider-URLs bleiben im JSON persistiert; `path` ist seit S3B die geprüfte Identität des Anhangs. **Aus den relationalen Zeilen wird keine Anhangliste rekonstruiert** — sie tragen weder `src` noch Reihenfolge.
