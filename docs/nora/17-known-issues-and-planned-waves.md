...
- **Verwaiste Storage-Objekte** — `ACCEPTED LIMITATION` seit W8-B. Es gibt derzeit **keinen** physischen Löschpfad: das Löschen einer Notiz (auch per CASCADE) entfernt die Datei nicht. Bestehende Verwaisungskandidaten wurden von W8-B **nicht** bereinigt. Ein neuer Löschpfad gehört in die Attachment-Foundation (W8-C) und wird neu entworfen — der alte Trigger-/`pg_net`-/Edge-Pfad wird nicht reaktiviert.
- **Kein Attachment-Modell** — `PLANNED DOMAIN WAVE` (W8-C), nicht begonnen. Anhänge leben als JSON-Array am Notizdatensatz: keine eigene Entität, keine Attachment-Id, keine Ownership auf Datensatzebene, keine Audit-Spur für das Hinzufügen oder Entfernen eines Anhangs. Der vom Client gelieferte `src`/`path` bleibt ein Legacy-Feld, und Provider-URLs bleiben persistiert.
