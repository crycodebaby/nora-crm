...
### H.1 Anhänge / Storage — offen nach W8-B, W8-C S1, S2A1, S2A2.1 und S2A2.2 (2026-09-18)
...
Ebenfalls **nicht** offen ist **W8-C S1** (Attachment Schema Foundation, `PRODUCTION VERIFIED` 2026-09-17): die Metadatentabelle `public.attachments` existiert in Production — additiv, **leer**, **nicht** an die Anwendung angebunden. Contract [`22`](22-security-and-access.md) Abschnitt 6.6, Entscheidung [`06`](06-decision-log.md) „2026-09-16 – W8-C S1", Evidenz `releases/2026-09.md`.

Ebenfalls **nicht** offen ist **W8-C S2A1** (Attachment Deletion Capture, `PRODUCTION VERIFIED` 2026-09-17): das Löschen einer Zeile in `public.attachments` schreibt seitdem ein Lösch**vorhaben** in die private Warteschlange `nora_private.attachment_storage_deletion_queue` — fail-closed, ohne Konsumenten, ohne jeden Storage- oder Netzwerkaufruf, in Production leer. Contract [`22`](22-security-and-access.md) Abschnitt 6.7, Entscheidung [`06`](06-decision-log.md) „2026-09-17 – W8-C S2A1", Evidenz `releases/2026-09.md`. **Erfassung ist keine Löschung** — die physische Bereinigung bleibt unten offen.
...
- **Der Löschpfad ist in Schnitte zerlegt — sie werden nicht zusammengezogen.** Reihenfolge und Zustand:
  ...
  - **S3 Schreibvertrag der Anhangverweise** — `OPEN`. Dual-Write auf `public.attachments` plus Write-Guard / atomare Ownership der Referenzschreibung. Die Referenzschreibung und die Erfassung müssen gegen ein aktives Löschvorhaben serialisiert bzw. koordiniert werden. Erst das schließt das Re-Referenzierungs-Rennen und LOW-1 (beide unten); ohne S3 bleibt S2B gesperrt.
  - **S2B physischer Löschworker (Storage-`DELETE`)** — `BLOCKED / FUTURE`, bis S3 geschlossen ist. S2B bringt den echten Worker und erst mit ihm eine eng geschnittene `service_role`-Ausführungsgrenze, einen Pfad zu `done` sowie den Vertrag für externe Seiteneffekte (unten). Die bloße Existenz der Warteschlange oder ihres Ausführungsvertrags ist **kein** Grund, einen Worker scharf zu schalten.
- **Re-Referenzierungs-Rennen** — `OPEN`, durch S2A1, S2A2.1 und S2A2.2 **nicht** gelöst. Die heutigen Anhangverweise werden über den Legacy-JSON-Pfad geschrieben und gehören keinem atomaren DB-Schreibvertrag. Ein Objektschlüssel kann deshalb zwischen dem Erfassen des Vorhabens und einer späteren Ausführung erneut referenziert werden. Ein physischer Worker darf daher **nicht** aktiviert werden, nur weil ein Vorhaben vorliegt oder der Resolver `dead` meldet — ein Warteschlangeneintrag ist Vorhaben, ein Resolver-Ergebnis eine Momentbeobachtung, beides nie Erlaubnis ([`22`](22-security-and-access.md) Abschnitt 6.7 und 6.8). Der künftige Schreibpfad muss ein aktives Löschvorhaben bzw. einen laufenden Auftrag berücksichtigen, bevor physische Löschung sicher werden kann. Aufgelöst wird das Rennen von S3, nicht von S2A1, S2A2.1 oder S2A2.2.
- **LOW-1: verlorenes Löschvorhaben zwischen Erfassung und Inspektion** — `OPEN (LOW)`, **S3/S2B-Gate**. In der unabhängigen Review von S2A2.2 mit zwei echten Sitzungen reproduziert:
  1. Für Schlüssel K existiert ein aktives Löschvorhaben.
  2. K wird erneut referenziert.
  3. Diese neue Referenz wird gelöscht, während das Vorhaben noch aktiv ist.
  4. Die Erfassung dieser Löschung wird unterdrückt, weil schon ein aktives Vorhaben existiert.
  5. Eine Inspektion sieht die noch nicht committete Löschung nicht, beobachtet `live` und überführt das Vorhaben in `skipped_live`.
  6. Danach committet die Löschung der Referenz.

  Ergebnis: keine lebende Referenz **und** kein aktives Vorhaben mehr. **Folge: ein Storage-Objekt kann verwaist liegen bleiben — das Vorhaben geht verloren. Es wird nie ein referenziertes Objekt gelöscht, und es gehen keine Daten verloren.** Bevor physischer Storage-`DELETE` aktiviert wird, muss S3 Referenzschreibung und Erfassung mit einem aktiven Löschvorhaben serialisieren bzw. koordinieren. Die konkrete Lösung ist nicht entworfen.
- **Externe Seiteneffekte sind durch die Lease nicht abgegrenzt** — `OPEN`, **S3/S2B-Gate**, kein S2A2.2-Defekt. Die Lease aus S2A2.2 sichert nur **Datenbank**-Mutationen der Warteschlange ab. Sie beweist nicht, dass ein bereits gestarteter künftiger Storage-Aufruf endet, wenn die Lease in der Datenbank abläuft. Ein Nachfolger kann den Auftrag dann erneut beanspruchen, während der alte Aufruf noch wirkt. Physische Löschung braucht deshalb zusätzlich einen eigenen Sicherheitsvertrag für externe Seiteneffekte (S3/S2B), bevor ein Worker scharf geschaltet wird.
- **Das Attachment-Modell ist ein leeres Fundament, keine Fläche** — `PLANNED DOMAIN WAVE` (W8-C S3 ff.), S1, S2A1, S2A2.1 und S2A2.2 erledigt. Seit S1 existiert die Entität `public.attachments` mit Attachment-Id, Ownership auf Datensatzebene (Notiz-XOR) und provider-neutraler `storage_key`-Identität. **Noch nicht vorhanden und nicht begonnen:** Dual-Write, Migration der Bestandsanhänge, Umschalten der Leseseite, Abschaltung des Legacy-Schreibpfads, physische Storage-Bereinigung, atomare Notiz-RPC sowie eine Audit-Spur für das Hinzufügen oder Entfernen eines Anhangs. Bis dahin bleiben die JSON-Arrays am Notizdatensatz die live genutzte Darstellung, der vom Client gelieferte `src`/`path` ein Legacy-Feld und Provider-URLs persistiert.
