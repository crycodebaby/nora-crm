...
-- Additive and not wired into the application: the table is EMPTY and nothing in
-- src/** reads or writes it; contact_notes.attachments / deal_notes.attachments
-- stay the live representation until a later slice. Deleting a row (directly or
-- by cascade) removes METADATA ONLY — S1 has no physical storage delete path.
