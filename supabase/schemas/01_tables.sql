...
-- Nora CRM W8-C S1 (2026-09-16): attachment metadata foundation.
-- Migration: 20260916120000_nora_attachment_foundation.sql
-- Since S3B the database is the single writer of this table (the note tables'
-- AFTER-ROW projection), since S4 it carries the backfilled historical rows,
-- and since S5 every note read embeds three of its columns — so neither half of
-- the original S1 note ("the table is EMPTY and nothing in src/** reads or
-- writes it") is true any more. contact_notes.attachments /
-- deal_notes.attachments stay the live order master for the READ path until
-- S6-B1. Deleting a row (directly or by cascade) removes METADATA ONLY — the
-- physical object is removed through the S2A1 deletion queue.
-- No uploaded_by (P-2 deferred), no lifecycle columns, no updated_at: attachment
-- metadata is conceptually immutable (no UPDATE policy, no UPDATE privilege).
-- W8-C S6-A (2026-09-22) added `ordinal`.
-- Migration: 20260922140000_nora_attachment_ordinal.sql
create table public.attachments (
    ...
    byte_size bigint,
    created_at timestamp with time zone not null default now(),
    -- S6-A: append sequence within the owning note, 1-based, append-only.
    -- v_base + element_no on every ADD, never renumbered, never compacted —
    -- gaps are normal. NOT the JSON array position.
    ...
    -- S6-A: one ordinal per note. NULLS NOT DISTINCT is required, not
    -- stylistic: exactly one owner column is non-NULL per row, so a plain
    -- UNIQUE over these three columns would be vacuous. NOT DEFERRABLE.
    constraint uq__attachments__owner_ordinal
        unique nulls not distinct (contact_note_id, deal_note_id, ordinal)
);

-- One storage object = one attachment identity (P-1: 31/31 distinct keys in Production).
...
-- Nora CRM W8-C S2A1 (2026-09-17): attachment deletion capture outbox.
-- Migration: 20260917120000_nora_attachment_deletion_capture.sql
-- Operational infrastructure, not a business entity: nora_private is not
-- PostgREST-exposed and no API role holds a single privilege here. A row is a
-- deletion INTENT captured from public.attachments, never a permission to
-- remove an object: nothing may delete a storage object before a consumer has
-- proven the key is referenced nowhere (legacy note JSON arrays, company logos
-- and the URL-only branding logos included). S2A1 has no consumer at all.
-- Idempotency is scoped to ACTIVE jobs via a PARTIAL unique index; a permanent
-- UNIQUE(storage_key) would strand a legitimately re-referenced key.
...
-- At most one ACTIVE job per storage key; terminal history stays unconstrained.
...
-- Defense in depth behind the missing grants: no policy exists, so RLS denies
-- every non-owner row access outright.
