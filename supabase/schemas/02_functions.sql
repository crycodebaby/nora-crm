...
-- takes nora_private.lock_attachment_storage_key(OLD.storage_key) before the
-- enqueue (lock order: attachment row -> key lock -> queue row).
-- SECURITY DEFINER is required, not stylistic: the deleting caller holds no
-- privilege on the private outbox (since S3A the API-reachable path is the FK
-- cascade of a note / contact / deal / company delete).
...
--
-- W8-C S3A (2026-09-19): reads the job's storage_key WITHOUT a row lock, takes
-- nora_private.lock_attachment_storage_key(key), and only then performs the
-- authoritative leased FOR UPDATE and the single resolver call - the liveness
-- observation can no longer race a reference deletion / admission (LOW-1).
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- W8-C S3A (2026-09-19) Storage-key serialization & reference admission
-- Migration: 20260919120000_nora_attachment_reference_serialization.sql
--
-- (a) the single per-storage-key coordination point (transaction-scoped
--     advisory lock, READ COMMITTED only). Callers: capture and admission
--     (AFTER row triggers on public.attachments: row -> key lock) and inspect
--     (key lock -> queue row). Never take it before an attachment INSERT/DELETE.
-- ---------------------------------------------------------------------------
create or replace function nora_private.lock_attachment_storage_key(p_storage_key text)
returns void
language plpgsql
volatile
security invoker
...
-- ---------------------------------------------------------------------------
-- W8-C S3A (2026-09-19) (b) Reference admission guard (invariant I1)
-- AFTER INSERT on public.attachments: no new reference to a key with an
-- ACTIVE deletion intent (pending / claimed / failed_retryable) or a done
-- tombstone -> 55000 NORA_ATTACHMENT_STORAGE_KEY_PENDING_DELETION. The queue
-- row is never modified.
-- ---------------------------------------------------------------------------
create or replace function nora_private.guard_attachment_reference_admission()
returns trigger
language plpgsql
security definer
set search_path = ''
...
-- ---------------------------------------------------------------------------
-- W8-C S3A (2026-09-19) (c) storage_key immutability (BEFORE UPDATE)
-- ---------------------------------------------------------------------------
create or replace function nora_private.guard_attachment_storage_key_immutable()
returns trigger
language plpgsql
security invoker
set search_path = ''
