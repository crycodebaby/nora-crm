...
--   * Append-only at runtime: INSERT + SELECT grants only. No application
--     role can update or delete a row. The table OWNER deliberately keeps
--     DELETE, so a later privileged User-Lifecycle purge (hard-deleting a test
--     account and every personal fragment of it) stays possible through a
--     SECURITY DEFINER function. That purge is NOT part of V1C-A.
...
-- never be updated.
--
-- Could a stale employee_sale_id ever resolve to a *different* employee?
-- public.sales.id is an identity column whose sequence only ever moves forward
-- and is never rewound by a DELETE, and no Nora code path inserts an explicit
-- id — so in practice, no. It is not structurally guaranteed, because the
-- column is GENERATED BY DEFAULT (an explicit id could be supplied by hand).
-- recipient_email_snapshot is the independent cross-check that makes such a
...
create table if not exists public.email_delivery_events (
    ...
    -- Correlation ----------------------------------------------------------
    -- Snapshot of the address the provider reported, lowercased so that dedupe
    -- and reporting are stable regardless of how the provider echoes it back.
    -- It is a snapshot on purpose: it must stay readable after the sales row is
    -- gone, and it is what lets a reader notice a stale employee_sale_id.
    ...
    -- Minimal diagnostics --------------------------------------------------
    -- The provider's own failure text, bounded. Enough for an administrator to
    -- act ("Adresse prüfen"), never a payload dump: no body, no headers, no
    -- link, no token.
    ...
    constraint email_delivery_events_recipient_len_check
        check (char_length(recipient_email_snapshot) between 3 and 320),
    constraint email_delivery_events_provider_reason_len_check
        check (provider_reason is null or char_length(provider_reason) <= 500),
...
comment on column public.email_delivery_events.recipient_email_snapshot is
    'Recipient address as reported by the provider, lowercased. Snapshot: survives deletion of the sales row and cross-checks employee_sale_id.';

comment on column public.email_delivery_events.provider_reason is
    'Bounded provider failure text for diagnostics. Never a payload dump, body, link or token.';
...
begin
    ...
    values (
        ...
        left(nullif(btrim(p_provider_reason), ''), 500),
        p_event_at,
        p_dedupe_key
    )
