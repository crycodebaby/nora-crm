...
declare
    ...
    v_uuid uuid;
begin
    -- 1) Transaction-local override (Edge / RPC SQL sessions)
    begin
...
comment on function nora_private.current_operation_id() is
    'Correlation helper: nora.operation_id GUC or request header x-nora-operation-id. Returns UUID text or NULL. Never used for auth/RLS.';
...
begin
    ...
    -- Soft correlation: NULL if missing/invalid — never fails the write
